...
-- Native POS sales periods are rebuilt whenever new paid checks land. These
-- lightweight triggers keep those rebuilds refund-aware so a later sale on the
-- same business day cannot restore pre-refund net sales.
DROP TRIGGER IF EXISTS trg_sales_periods_native_refunds_bi;
CREATE TRIGGER trg_sales_periods_native_refunds_bi BEFORE INSERT ON sales_periods FOR EACH ROW SET NEW.refunds_amount=IF(NEW.source_provider='gelato_pos' AND NEW.granularity='daily' AND NEW.service_period='all' AND NEW.location_id IS NOT NULL AND NEW.period_start=NEW.period_end,COALESCE((SELECT SUM(pr.amount) FROM pos_refunds pr JOIN pos_checks pc ON pc.id=pr.pos_check_id AND pc.organization_id=pr.organization_id WHERE pr.organization_id=NEW.organization_id AND pr.location_id=NEW.location_id AND pc.business_date=NEW.period_start AND pr.status='recorded'),0),NEW.refunds_amount),NEW.net_sales=IF(NEW.source_provider='gelato_pos' AND NEW.granularity='daily' AND NEW.service_period='all' AND NEW.location_id IS NOT NULL AND NEW.period_start=NEW.period_end,GREATEST(0,COALESCE((SELECT SUM(GREATEST(0,pc.subtotal-pc.discount_amount)) FROM pos_checks pc WHERE pc.organization_id=NEW.organization_id AND pc.location_id=NEW.location_id AND pc.business_date=NEW.period_start AND pc.status='paid'),0)-COALESCE((SELECT SUM(pr.amount) FROM pos_refunds pr JOIN pos_checks pc2 ON pc2.id=pr.pos_check_id AND pc2.organization_id=pr.organization_id WHERE pr.organization_id=NEW.organization_id AND pr.location_id=NEW.location_id AND pc2.business_date=NEW.period_start AND pr.status='recorded'),0)),NEW.net_sales);

DROP TRIGGER IF EXISTS trg_sales_periods_native_refunds_bu;
CREATE TRIGGER trg_sales_periods_native_refunds_bu BEFORE UPDATE ON sales_periods FOR EACH ROW SET NEW.refunds_amount=IF(NEW.source_provider='gelato_pos' AND NEW.granularity='daily' AND NEW.service_period='all' AND NEW.location_id IS NOT NULL AND NEW.period_start=NEW.period_end,COALESCE((SELECT SUM(pr.amount) FROM pos_refunds pr JOIN pos_checks pc ON pc.id=pr.pos_check_id AND pc.organization_id=pr.organization_id WHERE pr.organization_id=NEW.organization_id AND pr.location_id=NEW.location_id AND pc.business_date=NEW.period_start AND pr.status='recorded'),0),NEW.refunds_amount),NEW.net_sales=IF(NEW.source_provider='gelato_pos' AND NEW.granularity='daily' AND NEW.service_period='all' AND NEW.location_id IS NOT NULL AND NEW.period_start=NEW.period_end,GREATEST(0,COALESCE((SELECT SUM(GREATEST(0,pc.subtotal-pc.discount_amount)) FROM pos_checks pc WHERE pc.organization_id=NEW.organization_id AND pc.location_id=NEW.location_id AND pc.business_date=NEW.period_start AND pc.status='paid'),0)-COALESCE((SELECT SUM(pr.amount) FROM pos_refunds pr JOIN pos_checks pc2 ON pc2.id=pr.pos_check_id AND pc2.organization_id=pr.organization_id WHERE pr.organization_id=NEW.organization_id AND pr.location_id=NEW.location_id AND pc2.business_date=NEW.period_start AND pr.status='recorded'),0)),NEW.net_sales);

DROP TRIGGER IF EXISTS trg_sales_hourly_native_refunds_bi;
CREATE TRIGGER trg_sales_hourly_native_refunds_bi BEFORE INSERT ON sales_hourly FOR EACH ROW SET NEW.net_sales=IF(NEW.source_provider='gelato_pos' AND NEW.location_id IS NOT NULL,GREATEST(0,COALESCE((SELECT SUM(GREATEST(0,pc.subtotal-pc.discount_amount)) FROM pos_checks pc WHERE pc.organization_id=NEW.organization_id AND pc.location_id=NEW.location_id AND pc.business_date=NEW.business_date AND pc.closed_hour=NEW.hour_start AND pc.status='paid'),0)-COALESCE((SELECT SUM(pr.amount) FROM pos_refunds pr JOIN pos_checks pc2 ON pc2.id=pr.pos_check_id AND pc2.organization_id=pr.organization_id WHERE pr.organization_id=NEW.organization_id AND pr.location_id=NEW.location_id AND pc2.business_date=NEW.business_date AND pc2.closed_hour=NEW.hour_start AND pr.status='recorded'),0)),NEW.net_sales);

DROP TRIGGER IF EXISTS trg_sales_hourly_native_refunds_bu;
CREATE TRIGGER trg_sales_hourly_native_refunds_bu BEFORE UPDATE ON sales_hourly FOR EACH ROW SET NEW.net_sales=IF(NEW.source_provider='gelato_pos' AND NEW.location_id IS NOT NULL,GREATEST(0,COALESCE((SELECT SUM(GREATEST(0,pc.subtotal-pc.discount_amount)) FROM pos_checks pc WHERE pc.organization_id=NEW.organization_id AND pc.location_id=NEW.location_id AND pc.business_date=NEW.business_date AND pc.closed_hour=NEW.hour_start AND pc.status='paid'),0)-COALESCE((SELECT SUM(pr.amount) FROM pos_refunds pr JOIN pos_checks pc2 ON pc2.id=pr.pos_check_id AND pc2.organization_id=pr.organization_id WHERE pr.organization_id=NEW.organization_id AND pr.location_id=NEW.location_id AND pc2.business_date=NEW.business_date AND pc2.closed_hour=NEW.hour_start AND pr.status='recorded'),0)),NEW.net_sales);
