...
$pdo->prepare("INSERT INTO vendors (organization_id,public_id,name,status,created_by,updated_by) VALUES (?,?,'Other Supplier','active',?,?)")->execute([$org,'vendor-other-'.$slug,$uid,$uid]);$otherVendor=(int)$pdo->lastInsertId();
$pdo->prepare("INSERT INTO vendor_price_history (organization_id,vendor_id,inventory_item_id,price_per_pack,pack_size,unit,source_type,effective_at,created_by) VALUES (?,?,?,30,5,'lb','manual','2026-09-12 00:00:00',?)")->execute([$org,$otherVendor,(int)$flour['id'],$uid]);

$report=sales_cost_report($pdo,$org,'2026-09-01','2026-09-30',$location);$t=$report['totals'];scm_assert(abs((float)$t['netSales']-100)<0.01,'Net sales should be $100.');scm_assert(abs((float)$t['theoreticalCogsKnownItems']-16.50)<0.01,'Theoretical COGS should be $16.50.');scm_assert(abs((float)$t['grossProfitKnownItemsBeforeLaborOverhead']-83.50)<0.01,'Gross profit before labor/overhead should be $83.50.');scm_assert(abs((float)$t['grossMarginKnownItemsPercent']-83.50)<0.01,'Gross margin should be 83.5%.');scm_assert(abs((float)$t['costCoveragePercent']-100)<0.01,'Cost coverage should be 100%.');scm_assert(abs((float)$t['purchaseReceipts']-12)<0.01,'Received-purchase spend should use actual pack price and equal $12.');scm_assert(abs((float)$t['wasteCurrentCost']-4)<0.01,'Waste current cost should equal $4.');scm_assert(count($report['priceDrift'])===1&&$report['priceDrift'][0]['vendor']==='CI Foods'&&abs((float)$report['priceDrift'][0]['changePercent']-20)<0.01,'Vendor unit cost drift must compare the same item and vendor, not cross-vendor prices.');scm_assert(str_contains($report['disclaimer'],'not period COGS'),'Report must distinguish purchase spend from COGS.');
