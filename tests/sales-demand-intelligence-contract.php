...
$membership=(int)$pdo->query('SELECT LAST_INSERT_ID()')->fetchColumn();
...
foreach(['2026-08-29','2026-09-05','2026-09-12','2026-09-19'] as $date){$pdo->prepare("INSERT INTO time_clock_entries (organization_id,public_id,user_id,clocked_in_at,clocked_out_at,clock_in_source,clock_out_source,status,created_by,updated_by) VALUES (?,?,?,? ,?,'manual','manual','closed',?,?)")->execute([$org,sales_public_id('clock'),$user,$date.' 12:00:00',$date.' 20:00:00',$user,$user]);}
