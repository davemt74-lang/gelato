...
$pdo->prepare("INSERT INTO time_clock_entries (organization_id,public_id,user_id,clocked_in_at,clocked_out_at,status,created_by,updated_by) VALUES (?,'clock-perf',?,DATE_SUB(NOW(6),INTERVAL 2 DAY),DATE_ADD(DATE_SUB(NOW(6),INTERVAL 2 DAY),INTERVAL 8 HOUR),'closed',?,?)")->execute([$org,$employee,$manager,$manager]);
