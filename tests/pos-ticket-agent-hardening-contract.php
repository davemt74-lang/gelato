...
$paths=[
    ...
    'removeCore'=>$root.'/includes/pos-item-actions.php',
...
$checks=[
    ...
    'remove endpoint delegates to shared core'=>str_contains($files['removeApi'],'pos_item_remove_unsent'),
    'shared core blocks captured tender edits'=>str_contains($files['removeCore'],"status='captured'")&&str_contains($files['removeCore'],'pos_item_action_assert_unpaid'),
    'shared core blocks kitchen-sent line'=>str_contains($files['removeCore'],'kds_assert_pos_line_mutable'),
    'shared core removes only active line'=>str_contains($files['removeCore'],"DELETE FROM pos_check_items")&&str_contains($files['removeCore'],"status='active'"),
    'shared core recalculates ticket'=>str_contains($files['removeCore'],'pos_recalculate_check'),
    'shared core audits action'=>str_contains($files['removeCore'],'pos.item_removed'),
    'cart removal does not require void permission'=>!str_contains($files['removeApi'],"pos.void")&&!str_contains($files['removeCore'],"pos.void"),
