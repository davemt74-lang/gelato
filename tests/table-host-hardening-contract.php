...
$floor='floor-'.$slug;$pdo->prepare("INSERT INTO floor_plans (organization_id,public_id,name,plan_json,width_ft,depth_ft,scale_px_per_ft,version,is_default,created_by,updated_by) VALUES (?,?,?,JSON_OBJECT(),40,30,24,1,1,?,?)")->execute([$org,$floor,'Dining',$user,$user]);
$section=table_service_section_save($pdo,$org,$location,['name'=>'Dining','sortOrder'=>1],$user);
$tableA=service_ops_managed_table_create_safe($pdo,$org,$location,['name'=>'A1','capacity'=>4,'shape'=>'square','sectionPublicId'=>$section['publicId'],'floorPlanId'=>$floor,'xFt'=>5,'yFt'=>5],$user);
$tableB=service_ops_managed_table_create_safe($pdo,$org,$location,['name'=>'B1','capacity'=>4,'shape'=>'square','sectionPublicId'=>$section['publicId'],'floorPlanId'=>$floor,'xFt'=>10,'yFt'=>5],$user);
...
$blocked=false;try{service_ops_managed_table_create_safe($pdo,$org,$location,['name'=>'Tag Clash','capacity'=>2,'assetTag'=>$tableA['asset']['assetTag']],$user);}catch(InvalidArgumentException){$blocked=true;}thc($blocked,'Duplicate active asset tags must be rejected before database failure.');
$blocked=false;try{service_ops_place_table($pdo,$org,$location,$tableA['publicId'],['floorPlanId'=>$floor,'xFt'=>0,'yFt'=>0,'rotationDeg'=>45],$user);}catch(InvalidArgumentException){$blocked=true;}thc($blocked,'Full rotated table footprint must remain inside floor bounds.');
$tableA=service_ops_place_table($pdo,$org,$location,$tableA['publicId'],['floorPlanId'=>$floor,'xFt'=>20,'yFt'=>12,'rotationDeg'=>0],$user);
$map=service_ops_canonical_map($pdo,$org,$location);$mapped=array_values(array_filter($map['tables'],static fn(array $t):bool=>$t['publicId']===$tableA['publicId']))[0]??null;
thc($mapped!==null&&abs((float)$mapped['xPercent']-50.0)<0.01&&abs((float)$mapped['yPercent']-40.0)<0.01,'Table Service map must project canonical equipment floor coordinates.');

$legacy=table_service_table_save($pdo,$org,$location,['name'=>'Legacy Unsafe','capacity'=>2,'sectionPublicId'=>$section['publicId'],'xPercent'=>25,'yPercent'=>50],$user);
...
$legacy=service_ops_sync_table_asset($pdo,$org,$location,$legacy['publicId'],$user);
thc($legacy['managedAsset']&&$legacy['asset']['floorPlanId']===$floor,'Legacy sync must create and place the canonical physical asset.');
thc(abs((float)$legacy['asset']['xFt']-10.0)<0.05&&abs((float)$legacy['asset']['yFt']-15.0)<0.05,'Legacy percentage coordinates must migrate to default-floor feet.');
$unmanaged=table_service_table_save($pdo,$org,$location,['name'=>'Unmanaged Combo','capacity'=>2,'sectionPublicId'=>$section['publicId']],$user);
$blocked=false;try{service_ops_combination_save($pdo,$org,$location,['name'=>'Unsafe combo','tablePublicIds'=>[$tableA['publicId'],$unmanaged['publicId']]],$user);}catch(InvalidArgumentException){$blocked=true;}thc($blocked,'Table combinations must not include unmanaged physical tables.');
...
$near=pos_clock($pdo,$org,$location)->modify('+30 minutes')->format('Y-m-d H:i:s');$availableNear=service_ops_availability($pdo,$org,$location,$near,2,90);
...
$waitOccupied=service_ops_reservation_create($pdo,$org,$location,['type'=>'waitlist','guestName'=>'Occupied Wait','partySize'=>2],$user);
$blocked=false;try{service_ops_reservation_assign_safe($pdo,$org,$waitOccupied['publicId'],[$tableA['publicId']],$user);}catch(InvalidArgumentException){$blocked=true;}thc($blocked,'An actively occupied table must not be preassigned to a waiting party.');
...
$tableC=service_ops_managed_table_create_safe($pdo,$org,$location,['name'=>'C1','capacity'=>4,'sectionPublicId'=>$section['publicId']],$user);
...
$res=service_ops_reservation_status_atomic($pdo,$org,$res['publicId'],'confirmed',$user);thc($res['status']==='confirmed','Booked reservation must confirm atomically.');
$blocked=false;try{service_ops_reservation_status_atomic($pdo,$org,$res['publicId'],'booked',$user);}catch(InvalidArgumentException){$blocked=true;}thc($blocked,'Confirmed reservation must not regress to booked.');
...
$wait1=service_ops_reservation_assign_safe($pdo,$org,$wait1['publicId'],[$tableC['publicId']],$user);thc(count($wait1['tables'])===1,'Waitlist party must accept a table assignment.');
$blocked=false;try{service_ops_reservation_assign_safe($pdo,$org,$wait2['publicId'],[$tableC['publicId']],$user);}catch(InvalidArgumentException){$blocked=true;}thc($blocked,'One physical table must not be preassigned to two active waitlist parties.');
$blocked=false;try{service_ops_reservation_status_atomic($pdo,$org,$wait2['publicId'],'confirmed',$user);}catch(InvalidArgumentException){$blocked=true;}thc($blocked,'Waitlist lifecycle must reject reservation-only confirmed status.');
