...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $currentSummary = ['sections'=>(int)$summary['sections'],'items'=>(int)$summary['items']];
...
$csrf = app_csrf_token();
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Menu Import · <?= app_escape((string)($user['organization_name'] ?? 'Restaurant Training')) ?></title><style>
...
</style></head><body><div class="shell"><div class="top"><div><p class="eyebrow">Owner tools</p><h1>Import current menu</h1><p>Pull the same live menu data used by Gelato Spot and load it into this installation's database.</p></div><a href="index.html#menu">← Back to workspace</a></div>
<?php if ($message): ?><div class="notice ok"><?= app_escape($message) ?></div><?php endif; ?><?php if ($error): ?><div class="notice bad"><?= app_escape($error) ?></div><?php endif; ?>
<div class="grid"><section class="card"><p class="eyebrow">Live source</p><h2>Gelato Spot REST menu</h2><p>No API key or menu-source setup is required. The application has the public REST endpoint built in. Importing updates matching menu sections and items, deactivates menu records no longer present in the current source, and keeps the training interface database-driven.</p><form method="post"><input type="hidden" name="csrf_token" value="<?= app_escape($csrf) ?>"><button class="btn btn-dark" type="submit">Import current Gelato Spot menu</button></form><div class="warning"><strong>MCP is separate.</strong> MCP is not required to import the menu or run training. It remains available for later agent-facing restaurant knowledge integration.</div></section><aside class="card"><p class="eyebrow">Current database</p><div class="meta"><div><small>Active sections</small><strong class="stat"><?= (int)$currentSummary['sections'] ?></strong></div><div><small>Active items</small><strong class="stat"><?= (int)$currentSummary['items'] ?></strong></div></div><h3>REST source</h3><div class="code"><?= app_escape((string)$settings['rest_url']) ?></div><h3>Optional MCP</h3><div class="code"><?= app_escape((string)$settings['mcp_url']) ?></div><?php if ($summary): ?><h3>Last import</h3><div class="code"><?= app_escape(json_encode($summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES)) ?></div><?php endif; ?></aside></div></div><script src="js/universal-admin-page-shell.js?v=20260915-2"></script></body></html>
