...
{
    if(($row=kds_sent_line($pdo,$org,$posItemId))!==null)throw new InvalidArgumentException('This item has already been sent to the kitchen. Void it and add a corrected item instead.');
}
...
{
    $valid=['held'=>['queued','cancelled'],'queued'=>['held','in_progress','cancelled'],'in_progress'=>['ready','cancelled'],'ready'=>['in_progress','completed','cancelled'],'completed'=>[],'cancelled'=>[]];if(!array_key_exists($to,$valid))throw new InvalidArgumentException('Kitchen status is invalid.');$row=kds_item($pdo,$org,$publicId,true);$from=(string)$row['status'];if(!in_array($to,$valid[$from]??[],true))throw new InvalidArgumentException('Kitchen item cannot move from '.$from.' to '.$to.'.');if($row['station_id']===null&&!in_array($to,['cancelled'],true))throw new InvalidArgumentException('Route this item to an active kitchen station before changing its kitchen status.');
