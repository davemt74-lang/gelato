...
{
    kds_location($pdo,$org,$locationId);$public=trim((string)($input['publicId']??''));$name=mb_substr(trim((string)($input['name']??'')),0,120,'UTF-8');if($name==='')throw new InvalidArgumentException('Kitchen station name is required.');$slug=kds_slug((string)($input['slug']??$name));if($slug==='')throw new InvalidArgumentException('Kitchen station slug is invalid.');$target=max(30,min(7200,(int)($input['targetSeconds']??600)));$sort=(int)($input['sortOrder']??0);$status=(string)($input['status']??'active');if(!in_array($status,['active','inactive'],true))$status='active';
