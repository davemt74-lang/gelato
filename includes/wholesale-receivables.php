...
function wholesale_receivables_release_sales_lock(PDO $pdo,?string $lockName): void
{
...
{
    $salesLock=null;$changed=false;$issue=null;$due=null;
    $pdo->beginTransaction();
    try{
        $invoice=wholesale_receivables_invoice($pdo,$org,$invoicePublic,true);
        if($invoice['status']==='void')throw new InvalidArgumentException('Voided invoices cannot be issued.');
        if($invoice['status']!=='draft'){
            $issue=trim((string)$invoice['issue_date']);
            if($issue!=='')$salesLock=wholesale_receivables_sync_sales_date($pdo,$org,$issue,true);
            $pdo->commit();
        }else{
...
function wholesale_receivables_sync_sales_date(PDO $pdo,int $org,string $date,bool $keepLock=false): ?string
{
    if(!sales_intelligence_ready($pdo))return null;
    $date=wholesale_receivables_date($date,'Sales date');$provider=WHOLESALE_SALES_PROVIDER;$lockName='gelato:wholesale-sales:'.$org.':'.$date;$lock=null;$ownsTransaction=!$pdo->inTransaction();
    try{
        $q=$pdo->prepare('SELECT GET_LOCK(?,5)');$q->execute([$lockName]);if((int)$q->fetchColumn()!==1)throw new RuntimeException('Wholesale sales projection is busy. Retry the financial action.');$lock=$lockName;
        ...
        $cq=$pdo->prepare("SELECT e.amount_delta,i.invoice_number,i.subtotal,i.delivery_fee,i.tax_total,i.total FROM wholesale_receivable_entries e JOIN wholesale_invoices i ON i.id=e.wholesale_invoice_id AND i.organization_id=e.organization_id WHERE e.organization_id=? AND e.entry_type='credit' AND e.effective_date=? AND i.status<>'void'");$cq->execute([$org,$date]);$credits=$cq->fetchAll();
        if($invoices||$credits){
            $gross=0.0;$net=0.0;$tax=0.0;$fees=0.0;$refundRevenue=0.0;$items=[];$lineQ=$pdo->prepare('SELECT sku_snapshot,item_name_snapshot,quantity,line_subtotal FROM wholesale_invoice_items WHERE organization_id=? AND wholesale_invoice_id=? ORDER BY line_number');
        ...
        if($keepLock)return $lock;
    }catch(Throwable $e){if($ownsTransaction&&$pdo->inTransaction())$pdo->rollBack();if($lock!==null)wholesale_receivables_release_sales_lock($pdo,$lock);throw $e;}
    if($lock!==null)wholesale_receivables_release_sales_lock($pdo,$lock);return null;
}
