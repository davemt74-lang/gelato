...
function catering_brain_title(array $lead):string
{
    $event=trim((string)($lead['event_name']??''));
    if($event!=='')return $event;
    $company=trim((string)($lead['company_name']??''));
    if($company!=='')return $company;
    return trim((string)($lead['contact_name']??'Catering event'))?:'Catering event';
}
...
function catering_brain_text(PDO $pdo,int $organizationId,array $lead):string
{
    ...
    $lines=[
        'Catering opportunity: '.catering_brain_title($lead),
    ...
    if($activities){
        ...
        foreach($activities as $activity)$lines[]='- '.$activity['created_at'].' '.$activity['activity_type'].': '.$activity['summary'].(($activity['details']??'')?' — '.$activity['details']:'');
    }
    return implode("\n",$lines);
}
...
function restaurant_brain_sync_catering(PDO $pdo,int $organizationId,int $leadId,?int $userId):void
{
    ...
    if(!$lead)return;
    restaurant_brain_write_knowledge($pdo,$organizationId,'catering_lead',(string)$lead['public_id'],'Catering: '.catering_brain_title($lead),catering_brain_text($pdo,$organizationId,$lead),$userId);
}
