...
function table_turn_reset_metrics(PDO $pdo,int $org,int $locationId,string $date): array
{
    ...
    // service_events.created_at is generated by the database session clock. UNIX_TIMESTAMP
    // normalizes that wall time to an absolute instant before comparing it with the
    // restaurant location's local-day boundaries, including UTC/date and DST crossings.
    $q=$pdo->prepare("SELECT metadata_json FROM service_events WHERE organization_id=? AND location_id=? AND event_type='table_ready' AND UNIX_TIMESTAMP(created_at)>=? AND UNIX_TIMESTAMP(created_at)<? ORDER BY created_at,id");
    $q->execute([$org,$locationId,$start->getTimestamp(),$end->getTimestamp()]);
    $reset=[];$clean=[];
