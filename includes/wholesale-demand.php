...
function wholesale_demand_forecast(PDO $pdo,int $org,int $days=7): array
{
    ...
    $q=$pdo->prepare("SELECT i.public_id inventory_id,i.name,i.base_unit,i.on_hand_quantity,i.par_level,i.reorder_point,COALESCE(SUM(c.quantity),0) committed_quantity,COUNT(DISTINCT c.source_parent_public_id) order_count,MIN(c.commitment_date) next_commitment_date,MAX(c.commitment_date) last_commitment_date FROM inventory_items i LEFT JOIN inventory_commitments c ON c.organization_id=i.organization_id AND c.inventory_item_id=i.id AND c.status='active' AND c.commitment_date<=DATE_ADD(CURDATE(),INTERVAL ? DAY) WHERE i.organization_id=? AND i.archived_at IS NULL AND i.status='active' GROUP BY i.id HAVING committed_quantity>0 ORDER BY committed_quantity DESC,i.name");$q->execute([$days,$org]);$rows=[];$totals=['items'=>0,'orders'=>0,'committed'=>0.0,'shortages'=>0];
    foreach($q->fetchAll() as $r){$on=(float)$r['on_hand_quantity'];$committed=(float)$r['committed_quantity'];$available=$on-$committed;$target=$r['par_level']!==null?(float)$r['par_level']:($r['reorder_point']!==null?(float)$r['reorder_point']:0.0);$replenish=max(0,$target-$available);$shortage=max(0,-$available);$rows[]=['inventoryId'=>$r['inventory_id'],'name'=>$r['name'],'unit'=>$r['base_unit'],'onHand'=>$on,'committed'=>round($committed,4),'availableToPromise'=>round($available,4),'par'=>$r['par_level']!==null?(float)$r['par_level']:null,'reorderPoint'=>$r['reorder_point']!==null?(float)$r['reorder_point']:null,'replenishToTarget'=>round($replenish,4),'shortage'=>round($shortage,4),'orderCount'=>(int)$r['order_count'],'nextDate'=>$r['next_commitment_date'],'lastDate'=>$r['last_commitment_date']];$totals['items']++;$totals['committed']+=$committed;if($shortage>0)$totals['shortages']++;}
    $oq=$pdo->prepare("SELECT DISTINCT source_parent_public_id FROM inventory_commitments WHERE organization_id=? AND source_type='wholesale_order' AND status='active' AND commitment_date<=DATE_ADD(CURDATE(),INTERVAL ? DAY)");$oq->execute([$org,$days]);$totals['orders']=count($oq->fetchAll(PDO::FETCH_COLUMN));$totals['committed']=round($totals['committed'],4);
