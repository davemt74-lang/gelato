...
function sales_ensure_csv_integration(PDO $pdo,int $org,int $userId): void
{
    ...
    $hasPrimary=(int)$pdo->prepare('SELECT COUNT(*) FROM sales_integrations WHERE organization_id=? AND is_primary=1')->execute([$org]);
    $q2=$pdo->prepare('SELECT COUNT(*) FROM sales_integrations WHERE organization_id=? AND is_primary=1');$q2->execute([$org]);$primary=(int)$q2->fetchColumn()===0?1:0;
...
function sales_integration_save(PDO $pdo,int $org,array $input,int $userId): array
{
    ...
    $q=$pdo->prepare('SELECT id FROM sales_integrations WHERE organization_id=? AND provider=? AND '.($locationId?'location_id=?':'location_id IS NULL').' ORDER BY id LIMIT 1');$args=[$org,$provider];if($locationId)$args[]=$locationId;$q->execute($args);$id=(int)($q->fetchColumn()?:0);
    if($id){$pdo->prepare('UPDATE sales_integrations SET display_name=?,status=?,is_primary=?,restaurant_external_id=?,sync_enabled=?,updated_by=?,updated_at=NOW(6) WHERE id=? AND organization_id=?')->execute([$display,$provider==='csv'?'active':'available',$primary,$external,$sync,$userId,$id,$org]);}
...
{
    $fh=fopen($path,'rb');if(!$fh)return ',';$line=(string)fgets($fh);fclose($fh);$scores=[];foreach([','=>',','\t'=>'\t',';'=>';','|'=>'|'] as $k=>$delimiter){$d=$delimiter==='\t'?"\t":$delimiter;$scores[$d]=substr_count($line,$d);}arsort($scores);$best=(string)array_key_first($scores);return ($scores[$best]??0)>0?$best:',';
}
...
function sales_import_csv(PDO $pdo,int $org,int $userId,string $path,string $originalName,?int $locationId=null,bool $allowDuplicate=false): array
{
    ...
    try{$pdo->beginTransaction();while(($row=fgetcsv($fh,0,$delimiter))!==false){$rowNo++;if(count(array_filter($row,fn($v)=>trim((string)$v)!==''))===0)continue;$raw=[];foreach($headers as $i=>$h)$raw[(string)$h]=$row[$i]??'';$period=sales_period_values(sales_cell($row,$map,'date'),sales_cell($row,$map,'period_end'),sales_cell($row,$map,'granularity'));if(!$period){$rejected++;$reason='Invalid or missing period date.';$insRow->execute([$org,$batchId,$rowNo,'rejected',$reason,null,json_encode($raw,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)]);if(count($errors)<12)$errors[]='Row '.$rowNo.': '.$reason;continue;}
        $service=sales_service_period(sales_cell($row,$map,'service_period'));$location=sales_location($pdo,$org,sales_cell($row,$map,'location'),$locationId);$itemName=mb_substr(sales_cell($row,$map,'item_name'),0,240,'UTF-8');$rowType=mb_strtolower(sales_cell($row,$map,'row_type'),'UTF-8');$isItem=$itemName!==''||$rowType==='item';
        ...
        $periodCacheKey=implode('|',[$location['key'],$period['granularity'],$service,$period['start'],$period['end']]);if(!isset($touchedPeriods[$periodCacheKey])){$periodId=sales_period_upsert($pdo,$org,$location,'csv',$batchId,$period,$service,array_fill_keys(['tickets','covers'],0)+['gross'=>0,'net'=>0,'tax'=>0,'tips'=>0,'discounts'=>0,'comps'=>0,'voids'=>0,'refunds'=>0,'serviceCharges'=>0]);$pdo->prepare('DELETE FROM sales_item_periods WHERE organization_id=? AND sales_period_id=?')->execute([$org,$periodId]);$touchedPeriods[$periodCacheKey]=$periodId;}else $periodId=$touchedPeriods[$periodCacheKey];
        if(!$isItem){sales_period_upsert($pdo,$org,$location,'csv',$batchId,$period,$service,$metrics);$summarySeen[$periodCacheKey]=true;}
