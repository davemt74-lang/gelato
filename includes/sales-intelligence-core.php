...
function sales_import_csv(PDO $pdo,int $org,int $userId,string $path,string $originalName,?int $locationId=null,bool $allowDuplicate=false): array
{
    ...
    $insRow=$pdo->prepare('INSERT INTO sales_import_rows (organization_id,import_batch_id,source_row_number,row_status,rejection_reason,normalized_json,raw_json) VALUES (?,?,?,?,?,?,?)');
