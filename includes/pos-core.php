...
function pos_check_details(PDO $pdo,int $org,string $publicId): array
{
    ...
    $saleDue=pos_money((float)$c['subtotal']-(float)$c['discount_amount']+(float)$c['tax_amount']+(float)$c['service_charge_amount']);$salePaid=0.0;foreach($tenders->fetchAll() as $t)$allTenders[]=$t; $allTenders=$allTenders??[];foreach($allTenders as $t)if($t['status']==='captured')$salePaid+=(float)$t['amount'];
...
{
    $quantity=round($quantity,3);if($quantity<=0||$quantity>99)throw new InvalidArgumentException('Item quantity must be between 0.001 and 99.');$instructions=mb_substr(trim($instructions),0,1000,'UTF-8');$pdo->beginTransaction();try{$c=pos_require_open_check($pdo,$org,$publicId,true);$q=$pdo->prepare("SELECT p.id price_id,p.menu_item_id,p.option_name,p.amount,p.currency,i.name item_name,s.name category_name FROM menu_item_prices p JOIN menu_items i ON i.id=p.menu_item_id AND i.is_active=1 JOIN menu_sections s ON s.id=i.section_id AND s.organization_id=i.organization_id AND s.status='active' WHERE p.id=? AND i.organization_id=? AND (p.active_from IS NULL OR p.active_from<=NOW(6)) AND (p.active_until IS NULL OR p.active_until>NOW(6)) LIMIT 1");$q->execute([$priceId,$org]);$m=$q->fetch();if(!$m)throw new InvalidArgumentException('That menu price is not currently available.');$gross=pos_money((float)$m['amount']*$quantity);$pdo->prepare("INSERT INTO pos_check_items (organization_id,check_id,menu_item_id,menu_item_price_id,item_name_snapshot,option_name_snapshot,category_name_snapshot,quantity,unit_price,gross_amount,net_amount,special_instructions,status,added_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,'active',?)")->execute([$org,(int)$c['id'],(int)$m['menu_item_id'],$priceId,(string)$m['item_name'],(string)$m['option_name'],(string)$m['category_name'],$quantity,(float)$m['amount'],$gross,$gross,$instructions?:null,$userId]);pos_recalculate_check($pdo,$org,(int)$c['id']);$pdo->commit();return pos_check_details($pdo,$org,$publicId);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
}

function pos_update_item(PDO $pdo,int $org,string $publicId,int $itemId,float $quantity,string $instructions): array
{
    $quantity=round($quantity,3);if($quantity<=0||$quantity>99)throw new InvalidArgumentException('Item quantity must be between 0.001 and 99.');$instructions=mb_substr(trim($instructions),0,1000,'UTF-8');$pdo->beginTransaction();try{$c=pos_require_open_check($pdo,$org,$publicId,true);$q=$pdo->prepare("SELECT unit_price FROM pos_check_items WHERE organization_id=? AND check_id=? AND id=? AND status='active' LIMIT 1 FOR UPDATE");$q->execute([$org,(int)$c['id'],$itemId]);$unit=$q->fetchColumn();if($unit===false)throw new InvalidArgumentException('Open POS item was not found.');$gross=pos_money((float)$unit*$quantity);$pdo->prepare('UPDATE pos_check_items SET quantity=?,gross_amount=?,net_amount=?,special_instructions=?,updated_at=NOW(6) WHERE organization_id=? AND check_id=? AND id=?')->execute([$quantity,$gross,$gross,$instructions?:null,$org,(int)$c['id'],$itemId]);pos_recalculate_check($pdo,$org,(int)$c['id']);$pdo->commit();return pos_check_details($pdo,$org,$publicId);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
}

function pos_void_item(PDO $pdo,int $org,string $publicId,int $itemId,string $reason,int $userId): array
{
    $reason=mb_substr(trim($reason),0,500,'UTF-8');if($reason==='')throw new InvalidArgumentException('A void reason is required.');$pdo->beginTransaction();try{$c=pos_require_open_check($pdo,$org,$publicId,true);$q=$pdo->prepare("UPDATE pos_check_items SET status='voided',void_reason=?,voided_by=?,voided_at=NOW(6),updated_at=NOW(6) WHERE organization_id=? AND check_id=? AND id=? AND status='active'");$q->execute([$reason,$userId,$org,(int)$c['id'],$itemId]);if($q->rowCount()!==1)throw new InvalidArgumentException('Open POS item was not found.');pos_recalculate_check($pdo,$org,(int)$c['id']);$pdo->commit();return pos_check_details($pdo,$org,$publicId);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
}

function pos_apply_discount(PDO $pdo,int $org,string $publicId,float $amount,string $reason): array
{
    $amount=pos_money($amount);$reason=mb_substr(trim($reason),0,500,'UTF-8');if($amount>0&&$reason==='')throw new InvalidArgumentException('A discount reason is required.');$pdo->beginTransaction();try{$c=pos_require_open_check($pdo,$org,$publicId,true);pos_recalculate_check($pdo,$org,(int)$c['id']);$q=$pdo->prepare('SELECT subtotal FROM pos_checks WHERE organization_id=? AND id=?');$q->execute([$org,(int)$c['id']]);$subtotal=(float)$q->fetchColumn();if($amount>$subtotal)throw new InvalidArgumentException('Discount cannot exceed the current subtotal.');$pdo->prepare('UPDATE pos_checks SET discount_amount=?,discount_reason=?,revision=revision+1,updated_at=NOW(6) WHERE organization_id=? AND id=?')->execute([$amount,$amount>0?$reason:null,$org,(int)$c['id']]);pos_recalculate_check($pdo,$org,(int)$c['id']);$pdo->commit();return pos_check_details($pdo,$org,$publicId);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
}

function pos_cancel_check(PDO $pdo,int $org,string $publicId,string $reason,int $userId): array
{
    $reason=mb_substr(trim($reason),0,500,'UTF-8');if($reason==='')throw new InvalidArgumentException('A cancellation reason is required.');$pdo->beginTransaction();try{$c=pos_require_open_check($pdo,$org,$publicId,true);$q=$pdo->prepare("SELECT COUNT(*) FROM pos_tenders WHERE organization_id=? AND check_id=? AND status='captured'");$q->execute([$org,(int)$c['id']]);if((int)$q->fetchColumn()>0)throw new InvalidArgumentException('A check with captured payment cannot be cancelled; use a refund workflow.');$now=pos_clock($pdo,$org,(int)$c['location_id']);$pdo->prepare("UPDATE pos_checks SET status='cancelled',cancel_reason=?,cancelled_by=?,cancelled_at=?,revision=revision+1,updated_at=NOW(6) WHERE organization_id=? AND id=?")->execute([$reason,$userId,$now->format('Y-m-d H:i:s.u'),$org,(int)$c['id']]);$pdo->commit();return pos_check_details($pdo,$org,$publicId);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
}
