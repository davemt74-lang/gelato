...
function caa_operation_writable(array $operation): void
{
    if(in_array((string)($operation['status']??''),['completed','cancelled'],true)){
        throw new InvalidArgumentException('Completed or cancelled catering operations are read-only.');
    }
}
...
function caa_task_row(PDO $pdo,int $org,int $operationId,int $taskId): ?array
{
    ...
    $q->execute([$org,$operationId,$taskId]);
    $row=$q->fetch();
    return $row?:null;
}
...
function caa_requirement_row(PDO $pdo,int $org,int $operationId,int $requirementId): ?array
{
    ...
    $q->execute([$org,$operationId,$requirementId]);
    $row=$q->fetch();
    return $row?:null;
}
...
{
    $rows=catering_operation_menu($pdo,$org,$operationId);
    $normalized=[];
    foreach($rows as $row)$normalized[]=[
        'id'=>(int)$row['id'],
...
{
    if(!preg_match('/\b(?:mark|set|move)\s+(?:task\s+)?[“"\']?(.+?)[”"\']?\s+(?:as\s+|to\s+)?(done|complete|completed|in\s+progress|blocked|open|cancelled|canceled)\b/iu',$message,$m))return null;
    ...
    return ['query'=>trim((string)$m[1]),'status'=>$status];
}

function caa_requirement_status_from_message(string $message): ?array
{
    if(!preg_match('/\b(?:mark|set)\s+(?:ingredient|requirement)?\s*[“"\']?(.+?)[”"\']?\s+(?:as\s+|to\s+)?(ready|ordered|unavailable|planned)\b/iu',$message,$m))return null;
    return ['query'=>trim((string)$m[1]),'status'=>mb_strtolower(trim((string)$m[2]),'UTF-8')];
}

function caa_readiness_answer(PDO $pdo,int $org,array $operation): array
{
    $readiness=catering_operations_readiness($pdo,$org,(int)$operation['id'],true);
    $issues=(array)($readiness['issues']??[]);
    $answer=(string)$operation['title'].' is '.(int)$readiness['percent'].'% ready. '
        .(int)$readiness['taskDone'].' of '.(int)$readiness['taskTotal'].' tasks are done; '
        .(int)$readiness['requirementsReady'].' of '.(int)$readiness['requirementsTotal'].' ingredient requirements are ready; '
        .(int)$readiness['staffCount'].' staff are assigned.';
    if($issues)$answer.=' Attention: '.implode(' ',$issues);
    return ['ok'=>true,'skill'=>'catering.context_readiness','answer'=>$answer,'data'=>['operationId'=>$operation['public_id'],'readiness'=>$readiness],'sources'=>['Catering Operations']];
}

function caa_tasks_answer(PDO $pdo,int $org,array $operation): array
{
    $rows=catering_operation_tasks($pdo,$org,(int)$operation['id']);
    $open=array_values(array_filter($rows,static fn($row)=>!in_array((string)$row['status'],['done','cancelled'],true)));
    ...
    $parts=[];
    foreach(array_slice($open,0,10) as $row)$parts[]=$row['title'].' · '.str_replace('_',' ',(string)$row['status']).($row['due_at']?' · due '.$row['due_at']:'').($row['assigned_name']?' · '.$row['assigned_name']:' · unassigned');
...
function caa_requirements_answer(PDO $pdo,int $org,array $operation): array
{
    ...
    $attention=array_values(array_filter($rows,static fn($row)=>(string)$row['status']!=='ready'||$row['required_quantity']===null));
    $parts=[];
    ...
    $prefix=$attention?'Ingredient attention: ':'Ingredient requirements: ';
    return ['ok'=>true,'skill'=>'catering.context_requirements','answer'=>$prefix.implode('; ',$parts).'.','data'=>['operationId'=>$operation['public_id'],'requirements'=>count($rows),'attention'=>count($attention)],'sources'=>['Catering Operations → Ingredients']];
}
...
function caa_staff_answer(PDO $pdo,int $org,array $operation): array
{
    ...
    $parts=[];
    foreach(array_slice($rows,0,12) as $row)$parts[]=$row['user_name'].' · '.$row['role_name'].' · '.$row['status'].($row['shift_start_at']?' · '.$row['shift_start_at']:'');
...
function caa_execute_pending(PDO $pdo,int $org,int $uid,array $user,array $proposal): array
{
    ...
    $public=caa_clean_public_id($payload['operationPublicId']??'');
    ...
    caa_operation_writable($operation);
    $operationId=(int)$operation['id'];

    if($type==='task_add'){
        $title=mb_substr(trim((string)$payload['title']),0,240,'UTF-8');
        if($title==='')throw new InvalidArgumentException('The proposed catering task title is empty.');
        $pdo->prepare("INSERT INTO restaurant_operation_tasks (organization_id,operation_id,category,title,status,priority,created_by,updated_by) VALUES (?,?, 'operations',?,'open','normal',?,?)")
            ->execute([$org,$operationId,$title,$uid,$uid]);
    ...
    if($type==='task_status'){
        $task=caa_task_row($pdo,$org,$operationId,(int)$payload['taskId']);
        if(!$task)throw new InvalidArgumentException('The proposed catering task no longer exists.');
        ...
        if(!in_array($next,['open','in_progress','blocked','done','cancelled'],true))throw new InvalidArgumentException('The proposed catering task status is no longer supported.');
        $pdo->prepare('UPDATE restaurant_operation_tasks SET status=?,updated_by=?,updated_at=NOW(6) WHERE id=? AND organization_id=? AND operation_id=?')->execute([$next,$uid,(int)$task['id'],$org,$operationId]);
    ...
    if($type==='requirement_status'){
        $row=caa_requirement_row($pdo,$org,$operationId,(int)$payload['requirementId']);
        if(!$row)throw new InvalidArgumentException('The proposed ingredient requirement no longer exists.');
        ...
        $next=(string)$payload['status'];
        if(!in_array($next,['planned','ordered','ready','unavailable'],true))throw new InvalidArgumentException('The proposed ingredient status is no longer supported.');
        $pdo->prepare('UPDATE restaurant_operation_requirements SET status=?,updated_by=?,updated_at=NOW(6) WHERE id=? AND organization_id=? AND operation_id=?')->execute([$next,$uid,(int)$row['id'],$org,$operationId]);
...
function catering_agent_actions_handle(PDO $pdo,array $user,array $input): array
{
    ...
    caa_require($user,'catering.view','Catering view permission is required.');
    caa_require($user,'catering.agent','Catering Agent permission is required.');
    $message=trim((string)($input['message']??''));
    if($message===''||mb_strlen($message,'UTF-8')>1800)throw new InvalidArgumentException('Ask Gelato a catering question no longer than 1,800 characters.');
    $context=is_array($input['pageContext']??null)?$input['pageContext']:[];
    $operation=caa_context_operation($pdo,$org,$context);
    $pending=gac_pending_get('catering',$org,$uid);
    ...
    if(gac_is_confirm($message)){
        ...
        gac_pending_clear('catering',$org,$uid);
        app_audit($pdo,$org,$uid,'catering.agent_action_confirmed','catering_agent_proposal',(string)$pending['id'],null,['type'=>$pending['type']]);
        return ['ok'=>true]+$result;
    }
    if(gac_is_cancel($message)&&$pending){
        gac_pending_clear('catering',$org,$uid);
        app_audit($pdo,$org,$uid,'catering.agent_action_discarded','catering_agent_proposal',(string)$pending['id'],null,['type'=>$pending['type']]);
    ...
    if($operation){
        if(preg_match('/\b(?:rebuild|regenerate|recalculate)\b.*\b(?:ingredient|requirements?)\b|\b(?:ingredient|requirements?)\b.*\b(?:rebuild|regenerate|recalculate)\b/iu',$message)){
            caa_require($user,'catering.manage','Catering management permission is required to rebuild ingredient requirements.');
            caa_operation_writable($operation);
            ...
            app_audit($pdo,$org,$uid,'catering.agent_action_proposed','catering_agent_proposal',(string)$proposal['id'],null,['type'=>'requirements_rebuild','operationId'=>$operation['public_id']]);
            return gac_proposal_result($proposal,'catering.action_proposal',['Catering Operations → Menu & Recipes','Catering Operations → Ingredients']);
        }

        if(preg_match('/\b(?:add|create|make)\s+(?:a\s+)?(?:catering\s+)?task\s*[:\-]?\s*(.+)$/iu',$message,$m)){
            caa_require($user,'catering.manage','Catering management permission is required to add catering tasks.');
            caa_operation_writable($operation);
            $title=mb_substr(trim((string)$m[1]," \t\n\r\0\x0B?.!"),0,240,'UTF-8');
            if($title==='')throw new InvalidArgumentException('Tell me the task title to add.');
            ...
            app_audit($pdo,$org,$uid,'catering.agent_action_proposed','catering_agent_proposal',(string)$proposal['id'],null,['type'=>'task_add','operationId'=>$operation['public_id']]);
            return gac_proposal_result($proposal,'catering.action_proposal',['Catering Operations → Tasks']);
        }

        $taskChange=caa_task_status_from_message($message);
        if($taskChange){
            caa_require($user,'catering.manage','Catering management permission is required to update catering tasks.');
            caa_operation_writable($operation);
            $task=caa_find_task($pdo,$org,(int)$operation['id'],$taskChange['query']);
            ...
            app_audit($pdo,$org,$uid,'catering.agent_action_proposed','catering_agent_proposal',(string)$proposal['id'],null,['type'=>'task_status','operationId'=>$operation['public_id'],'taskId'=>$task['id']]);
            return gac_proposal_result($proposal,'catering.action_proposal',['Catering Operations → Tasks']);
        }

        $requirementChange=caa_requirement_status_from_message($message);
        if($requirementChange){
            caa_require($user,'catering.manage','Catering management permission is required to update ingredient requirements.');
            caa_operation_writable($operation);
            $row=caa_find_requirement($pdo,$org,(int)$operation['id'],$requirementChange['query']);
            ...
            app_audit($pdo,$org,$uid,'catering.agent_action_proposed','catering_agent_proposal',(string)$proposal['id'],null,['type'=>'requirement_status','operationId'=>$operation['public_id'],'requirementId'=>$row['id']]);
            return gac_proposal_result($proposal,'catering.action_proposal',['Catering Operations → Ingredients']);
        }
    ...
    $result=(catering_operations_ready($pdo)&&catering_agent_operations_intent($message))?catering_operations_agent_answer($pdo,$org,$message):catering_brain_answer($pdo,$org,$message);
    return ['ok'=>true]+$result;
}
