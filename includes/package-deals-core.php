...
function package_deal_row(PDO $pdo,int $org,string $publicOrSlug,bool $forUpdate=false): array
{
    ...
    $q=$pdo->prepare($sql);
    $q->execute([$org,$publicOrSlug,$publicOrSlug]);
    $row=$q->fetch();
...
{
    $q=$pdo->prepare("SELECT g.id,g.public_id,g.label,g.required_quantity,g.sort_order,
                            gi.menu_item_price_id,p.option_name,p.size_code,p.amount,p.currency,p.active_from,p.active_until,
                            i.id menu_item_id,i.name item_name,i.description,i.is_active item_active,
                            s.name section_name,s.status section_status,
                            CASE WHEN i.is_active=1 AND s.status='active'
                                      AND (p.active_from IS NULL OR p.active_from<=NOW(6))
                                      AND (p.active_until IS NULL OR p.active_until>NOW(6))
                                 THEN 1 ELSE 0 END price_available
    ...
    foreach($q->fetchAll() as $r){
        ...
        if($r['menu_item_price_id']!==null){
            $groups[$gid]['items'][]=[
                'priceId'=>(int)$r['menu_item_price_id'],
                'itemId'=>(int)$r['menu_item_id'],
                ...
                'available'=>(bool)$r['price_available'],
            ];
        }
    }
    ...
    $redemptions->execute([$org,(int)$row['id']]);
    $stats=$redemptions->fetch()?:[];
    ...
    return [
        ...
        'discountMethod'=>(string)$row['discount_method'],
        'discountValue'=>(float)$row['discount_value'],
        'pickupOnly'=>(bool)$row['pickup_only'],
        ...
        'stats'=>[
            'redemptions'=>(int)($stats['redemptions']??0),
            'retail'=>(float)($stats['retail']??0),
            'discounts'=>(float)($stats['discounts']??0),
...
{
    $pickup=(bool)($row['pickup_only']??$row['pickupOnly']??false);
    if((string)($row['status']??'')!=='active' || !$pickup) return false;
    $starts=$row['starts_at']??$row['startsAt']??null;
    $ends=$row['ends_at']??$row['endsAt']??null;
    $now??=new DateTimeImmutable();
    if($starts && $now<new DateTimeImmutable((string)$starts)) return false;
    if($ends && $now>=new DateTimeImmutable((string)$ends)) return false;
    return true;
}

function package_deal_has_orderable_groups(array $package): bool
{
    if(empty($package['groups'])) return false;
    foreach($package['groups'] as $group){
        $available=array_filter((array)($group['items']??[]),static fn(array $item):bool=>!empty($item['available']));
...
function package_deal_public_list(PDO $pdo,int $org): array
{
    ...
    foreach($q->fetchAll() as $row){
        ...
        if(package_deal_has_orderable_groups($package))$out[]=$package;
    }
    return $out;
}
...
function package_deal_validate_groups(PDO $pdo,int $org,array $groups): array
{
    ...
    foreach($groups as $index=>$group){
        ...
        $priceIds=[];
        foreach((array)($group['priceIds']??[]) as $priceId){
            $id=(int)$priceId;
            if($id>0)$priceIds[$id]=$id;
        }
        ...
        sort($valid);
        $expected=array_values($priceIds);
        sort($expected);
...
function package_deal_save(PDO $pdo,int $org,?string $publicId,array $input,int $userId): array
{
    ...
    $starts=trim((string)($input['startsAt']??''))?:null;
    $ends=trim((string)($input['endsAt']??''))?:null;
    if($starts!==null)new DateTimeImmutable($starts);
    if($ends!==null)new DateTimeImmutable($ends);
    ...
    return pos_transaction($pdo,function()use($pdo,$org,$publicId,$input,$userId,$name,$eyebrow,$description,$method,$value,$groups,$starts,$ends):array{
        $existing=$publicId?package_deal_row($pdo,$org,$publicId,true):null;
        if($existing){
        ...
        foreach($groups as $group){
            ...
            foreach($group['priceIds'] as $itemIndex=>$priceId){
                $pdo->prepare('INSERT INTO package_deal_group_items (organization_id,package_deal_id,package_group_id,menu_item_price_id,sort_order) VALUES (?,?,?,?,?)')
                    ->execute([$org,$packageId,$groupId,$priceId,$itemIndex]);
            }
        }
        $q=$pdo->prepare('SELECT * FROM package_deals WHERE organization_id=? AND id=? LIMIT 1');
...
function package_deal_set_status(PDO $pdo,int $org,string $publicId,string $action,int $userId): array
{
    ...
    return pos_transaction($pdo,function()use($pdo,$org,$publicId,$action,$userId):array{
        $row=package_deal_row($pdo,$org,$publicId,true);
        ...
        $target=match($action){
            ...
            'end'=>'ended',
            'archive'=>'archived',
            default=>throw new InvalidArgumentException('Unknown package status action.'),
        };
        ...
        if($target==='active'){
            $package=package_deal_payload($pdo,$org,$row);
            if(!package_deal_has_orderable_groups($package)) throw new InvalidArgumentException('Every package category needs at least one currently orderable menu choice before the deal can go active.');
        }

        $pausedSql=$target==='paused'?'NOW(6)':($target==='active'?'NULL':'paused_at');
        $endedSql=$target==='ended'?'NOW(6)':($target==='active'?'NULL':'ended_at');
        $archivedSql=$target==='archived'?'NOW(6)':'archived_at';
        $sql="UPDATE package_deals SET status=?,paused_at={$pausedSql},ended_at={$endedSql},archived_at={$archivedSql},updated_by=?,updated_at=NOW(6) WHERE organization_id=? AND id=?";
        $pdo->prepare($sql)->execute([$target,$userId,$org,(int)$row['id']]);
        return package_deal_payload($pdo,$org,package_deal_row($pdo,$org,$publicId,false));
    });
}
    ...
    return pos_transaction($pdo,function()use($pdo,$org,$publicId,$userId):array{
        ...
        foreach($source['groups'] as $group){
            $groups[]=[
                ...
                'priceIds'=>array_column($group['items'],'priceId'),
...
function package_deal_menu_search(PDO $pdo,int $org,string $query,int $limit=40): array
{
    ...
    $like='%'.$query.'%';
    $sql="SELECT p.id price_id,p.option_name,p.size_code,p.amount,p.currency,i.id item_id,i.name item_name,i.description,s.name section_name
          ...
          WHERE (i.name LIKE ? OR s.name LIKE ? OR p.option_name LIKE ?)
            AND (p.active_from IS NULL OR p.active_from<=NOW(6))
            AND (p.active_until IS NULL OR p.active_until>NOW(6))
          ORDER BY s.sort_order,s.name,i.name,p.sort_order,p.id LIMIT ".$limit;
    $q=$pdo->prepare($sql);
    $q->execute([$org,$like,$like,$like]);
...
function package_deal_selection_cart(PDO $pdo,int $org,array $package,array $selections): array
{
    ...
    foreach($package['groups'] as $group){
        ...
        foreach($group['items'] as $item){
            if(!empty($item['available']))$allowed[(int)$item['priceId']]=$item;
        }
        $counts=[];
        $selectedSnapshot=[];
        foreach($selected as $priceId){
            ...
            $counts[$priceId]=($counts[$priceId]??0)+1;
            $retail+=(float)$allowed[$priceId]['amount'];
            $selectedSnapshot[]=[
                'priceId'=>$priceId,
                'itemName'=>$allowed[$priceId]['itemName'],
                'optionName'=>$allowed[$priceId]['optionName'],
                'amount'=>$allowed[$priceId]['amount'],
            ];
        }
        foreach($counts as $priceId=>$qty){
            $cart[]=['priceId'=>$priceId,'quantity'=>$qty,'instructions'=>'PACKAGE: '.$package['name'].' · '.$group['label']];
        }
...
function package_deal_submit_pickup(PDO $pdo,int $org,array $account,array $input): array
{
    ...
    $row=package_deal_row($pdo,$org,(string)($input['package']??''),false);
    $package=package_deal_payload($pdo,$org,$row);
    if(!package_deal_is_public($package) || !package_deal_has_orderable_groups($package)) throw new InvalidArgumentException('That package deal is not currently available.');
    ...
    try{
        ...
        $oq=$pdo->prepare('SELECT id,pos_check_id FROM online_orders WHERE organization_id=? AND public_id=? LIMIT 1');
        $oq->execute([$org,(string)$order['public_id']]);
        $orderRow=$oq->fetch();
        if(!$orderRow) throw new RuntimeException('Package order could not be linked to the online order.');

        if(!empty($order['duplicate'])){
            $rq=$pdo->prepare('SELECT public_id,retail_amount,discount_amount,package_amount FROM package_redemptions WHERE organization_id=? AND online_order_id=? LIMIT 1');
            $rq->execute([$org,(int)$orderRow['id']]);
            $redemption=$rq->fetch();
            if(!$redemption) throw new RuntimeException('Existing package order is missing its package redemption record.');
            if($owns)$pdo->commit();
            return $order+['package'=>$package,'redemption'=>$redemption];
        }

        $discount=discount_apply(
            $pdo,$org,(string)$order['check_public_id'],'package_deal',$package['discountMethod'],(float)$package['discountValue'],
            'Package deal: '.$package['name'],(int)$account['id'],'package_deal',$package['id']
        );
        ...
        $pdo->prepare('INSERT INTO package_redemptions (organization_id,package_deal_id,online_order_id,pos_check_id,pos_discount_id,public_id,package_name_snapshot,retail_amount,discount_amount,package_amount,discount_method,discount_value,selection_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)')
            ->execute([
                $org,(int)$row['id'],(int)$orderRow['id'],(int)$orderRow['pos_check_id'],(int)$discountRow['id'],$redemptionPublic,
                $package['name'],$selection['retail'],(float)$discountRow['amount'],$packageAmount,$package['discountMethod'],$package['discountValue'],
                json_encode($selection['snapshot'],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR),
            ]);
        if($owns)$pdo->commit();
        return $order+[
            'subtotal'=>$check['subtotal'],
            'tax_amount'=>$check['taxAmount'],
            'service_charge_amount'=>$check['serviceChargeAmount'],
            'total_amount'=>$check['totalAmount'],
            'package'=>$package,
            'redemption'=>[
                'public_id'=>$redemptionPublic,
                'retail_amount'=>$selection['retail'],
                'discount_amount'=>(float)$discountRow['amount'],
