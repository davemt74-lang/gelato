    ...
    if(restaurant_brain_table_ready($pdo,'recipes')){
        ...
        foreach($stmt->fetchAll() as $recipe){
            ...
            foreach($ingredients as $ingredient){
                ...
                $item=operations_inventory_upsert($pdo,$organizationId,$parsed['name'],$userId);$itemsTouched[(int)$item['id']]=true;$sourceId='recipe:'.$recipe['public_id'];
                $source=$pdo->prepare("INSERT INTO inventory_item_sources (organization_id,inventory_item_id,source_type,source_public_id,source_name,quantity_per_source,unit,metadata_json) VALUES (?,?, 'recipe',?,?,?,?,?) ON DUPLICATE KEY UPDATE source_name=VALUES(source_name),quantity_per_source=VALUES(quantity_per_source),unit=VALUES(unit),metadata_json=VALUES(metadata_json),updated_at=NOW(6)");
                $source->execute([$organizationId,(int)$item['id'],$sourceId,$recipe['name'],$parsed['quantity'],$parsed['unit']?:null,json_encode(['recipePublicId'=>$recipe['public_id'],'recipeName'=>$recipe['name']],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR)]);$recipeCount++;
            }
        }
    }
...
function operations_inventory_adjust(PDO $pdo,int $organizationId,string $publicId,float $delta,string $type,?int $userId,string $note=''): array
{
    ...
    try{
        $stmt=$pdo->prepare('SELECT * FROM inventory_items WHERE organization_id=? AND public_id=? AND archived_at IS NULL FOR UPDATE');$stmt->execute([$organizationId,$publicId]);$item=$stmt->fetch();if(!$item)throw new RuntimeException('Inventory item not found.');
        $old=(float)$item['on_hand_quantity'];$new=$type==='count'?$delta:$old+$delta;if($new<0)$new=0;
        $pdo->prepare('UPDATE inventory_items SET on_hand_quantity=?,last_counted_at=IF(?="count",NOW(6),last_counted_at),updated_by=?,updated_at=NOW(6) WHERE id=?')->execute([$new,$type,$userId,(int)$item['id']]);
        $pdo->prepare('INSERT INTO inventory_transactions (organization_id,inventory_item_id,transaction_type,quantity_delta,resulting_quantity,unit,note,created_by) VALUES (?,?,?,?,?,?,?,?)')->execute([$organizationId,(int)$item['id'],$type,$type==='count'?$new-$old:$delta,$new,$item['base_unit'],$note?:null,$userId]);
        $pdo->commit();$item['on_hand_quantity']=$new;return $item;
...
function operations_sync_catering_tasks(PDO $pdo,int $organizationId,?int $userId=null): int
{
    ...
    foreach($stmt->fetchAll() as $source){
        $categorySlug=in_array((string)$source['category'],['prep','inventory','service','logistics','menu'],true)?((string)$source['category']==='logistics'?'delivery':((string)$source['category']==='menu'?'catering':(string)$source['category'])):'catering';$category=operations_category_by_slug($pdo,$organizationId,$categorySlug);$sourceId='catering-task-'.$source['id'];$statusMap=['open'=>'queued','in_progress'=>'in_progress','blocked'=>'blocked','done'=>'completed','cancelled'=>'cancelled'];$status=$statusMap[$source['status']]??'queued';$publicId='task-'.substr(hash('sha256',$organizationId.'|'.$sourceId),0,20);
        $upsert=$pdo->prepare("INSERT INTO restaurant_tasks (organization_id,public_id,category_id,source_type,source_public_id,title,description,priority,status,due_at,created_by,updated_by) VALUES (?,?,?,'catering_operation_task',?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE category_id=VALUES(category_id),title=VALUES(title),description=VALUES(description),priority=VALUES(priority),status=IF(status IN ('verified'),status,VALUES(status)),due_at=VALUES(due_at),updated_at=NOW(6)");
        $upsert->execute([$organizationId,$publicId,$category['id']??null,$sourceId,$source['title'],$source['description'],$source['priority'],$status,$source['due_at'],$userId,$userId]);$task=operations_task_by_public_id($pdo,$organizationId,$publicId);
        if($task&&!empty($source['assigned_to'])){$pdo->prepare("INSERT IGNORE INTO restaurant_task_assignments (organization_id,task_id,user_id,assignment_role,assigned_by) VALUES (?, ?, ?, 'assignee', ?)")->execute([$organizationId,(int)$task['id'],(int)$source['assigned_to'],$userId]);}$count++;
    }
    return $count;
}
...
{
    $task=operations_task_by_public_id($pdo,$organizationId,$publicId);if(!$task)return null;$id=(int)$task['id'];
    $stmt=$pdo->prepare("SELECT a.*,u.display_name FROM restaurant_task_assignments a INNER JOIN users u ON u.id=a.user_id WHERE a.organization_id=? AND a.task_id=? ORDER BY a.assignment_role,u.display_name");$stmt->execute([$organizationId,$id]);$task['assignments']=$stmt->fetchAll();
    $stmt=$pdo->prepare("SELECT e.*,u.display_name actor_name FROM restaurant_task_events e LEFT JOIN users u ON u.id=e.actor_user_id WHERE e.organization_id=? AND e.task_id=? ORDER BY e.created_at DESC LIMIT 100");$stmt->execute([$organizationId,$id]);$task['events']=$stmt->fetchAll();
    $stmt=$pdo->prepare("SELECT te.*,u.display_name FROM restaurant_task_time_entries te INNER JOIN users u ON u.id=te.user_id WHERE te.organization_id=? AND te.task_id=? ORDER BY te.started_at DESC");$stmt->execute([$organizationId,$id]);$task['timeEntries']=$stmt->fetchAll();
...
{
    $allowed=['queued','assigned','accepted','in_progress','blocked','completed','verified','cancelled'];if(!in_array($status,$allowed,true))throw new InvalidArgumentException('Invalid task status.');
    $task=operations_task_by_public_id($pdo,$organizationId,$publicId);if(!$task)throw new RuntimeException('Task not found.');
    $pdo->prepare("UPDATE restaurant_tasks SET status=?,verified_at=IF(?='verified',NOW(6),verified_at),verified_by=IF(?='verified',?,verified_by),updated_by=?,updated_at=NOW(6) WHERE id=? AND organization_id=?")
        ->execute([$status,$status,$status,$userId,$userId,(int)$task['id'],$organizationId]);
    if($status==='completed'||$status==='verified')$pdo->prepare('UPDATE restaurant_task_assignments SET completed_at=COALESCE(completed_at,NOW(6)) WHERE task_id=? AND user_id=?')->execute([(int)$task['id'],$userId]);
    if($task['source_type']==='catering_operation_task'&&preg_match('/catering-task-(\d+)/',(string)$task['source_public_id'],$m)){
        $map=['queued'=>'open','assigned'=>'open','accepted'=>'open','in_progress'=>'in_progress','blocked'=>'blocked','completed'=>'done','verified'=>'done','cancelled'=>'cancelled'];
        $pdo->prepare('UPDATE restaurant_operation_tasks SET status=?,updated_by=?,updated_at=NOW(6) WHERE id=? AND organization_id=?')->execute([$map[$status]??'open',$userId,(int)$m[1],$organizationId]);
    }
...
function operations_task_timer(PDO $pdo,int $organizationId,string $publicId,int $userId,bool $start): array
{
    ...
    if($start){
        $existing=$pdo->prepare('SELECT id FROM restaurant_task_time_entries WHERE organization_id=? AND task_id=? AND user_id=? AND ended_at IS NULL LIMIT 1');$existing->execute([$organizationId,$id,$userId]);
        if(!$existing->fetchColumn())$pdo->prepare('INSERT INTO restaurant_task_time_entries (organization_id,task_id,user_id,started_at) VALUES (?,?,?,NOW(6))')->execute([$organizationId,$id,$userId]);
        $pdo->prepare("UPDATE restaurant_tasks SET status=IF(status IN ('queued','assigned','accepted'),'in_progress',status),updated_by=?,updated_at=NOW(6) WHERE id=?")->execute([$userId,$id]);operations_task_event($pdo,$organizationId,$id,'timer_started','Task timer started',$userId);
    }else{
        $entry=$pdo->prepare('SELECT id,started_at FROM restaurant_task_time_entries WHERE organization_id=? AND task_id=? AND user_id=? AND ended_at IS NULL ORDER BY started_at DESC LIMIT 1');$entry->execute([$organizationId,$id,$userId]);$row=$entry->fetch();
        if($row)$pdo->prepare('UPDATE restaurant_task_time_entries SET ended_at=NOW(6),duration_seconds=GREATEST(0,TIMESTAMPDIFF(SECOND,started_at,NOW(6))) WHERE id=?')->execute([(int)$row['id']]);operations_task_event($pdo,$organizationId,$id,'timer_stopped','Task timer stopped',$userId);
    }
...
{
    operations_ensure_default_categories($pdo,$organizationId,null);operations_sync_catering_tasks($pdo,$organizationId,null);
    $stmt=$pdo->prepare('SELECT COUNT(*) FROM inventory_items WHERE organization_id=? AND archived_at IS NULL');$stmt->execute([$organizationId]);$inventoryCount=(int)$stmt->fetchColumn();
    if($inventoryCount===0){operations_sync_inventory_sources($pdo,$organizationId,null);$stmt->execute([$organizationId]);$inventoryCount=(int)$stmt->fetchColumn();}
    $stmt=$pdo->prepare('SELECT COUNT(*) FROM inventory_items WHERE organization_id=? AND archived_at IS NULL AND reorder_point IS NOT NULL AND on_hand_quantity<=reorder_point');$stmt->execute([$organizationId]);$low=(int)$stmt->fetchColumn();
    $stmt=$pdo->prepare("SELECT COUNT(*) total,SUM(status IN ('queued','assigned','accepted','in_progress','blocked')) open_count,SUM(status='in_progress') in_progress,SUM(status IN ('queued','assigned','accepted','in_progress','blocked') AND due_at<NOW()) overdue FROM restaurant_tasks WHERE organization_id=? AND archived_at IS NULL");$stmt->execute([$organizationId]);$tasks=$stmt->fetch()?:[];
    return ['inventoryCount'=>$inventoryCount,'lowStock'=>$low,'taskTotal'=>(int)($tasks['total']??0),'openTasks'=>(int)($tasks['open_count']??0),'inProgress'=>(int)($tasks['in_progress']??0),'overdue'=>(int)($tasks['overdue']??0)];
}

function operations_agent_inventory_answer(PDO $pdo,int $organizationId,string $message): array
{
    $normalized=mb_strtolower($message,'UTF-8');$low=preg_match('/\b(low|below par|reorder|short|shortage|running out)\b/u',$normalized)===1;$rows=operations_inventory_list($pdo,$organizationId,$low?'':$message,$low,40);if(!$rows&&$low)$rows=operations_inventory_list($pdo,$organizationId,'',true,40);
    if(!$rows)return ['skill'=>'inventory.search','answer'=>$low?'No inventory items are currently at or below their reorder point.':'I could not find a matching inventory item.','data'=>[],'sources'=>[]];
    $lines=[];foreach(array_slice($rows,0,25) as $row)$lines[]=$row['name'].' — on hand '.$row['on_hand_quantity'].($row['base_unit']?' '.$row['base_unit']:'').($row['par_level']!==null?' / par '.$row['par_level']:'').($row['reorder_point']!==null?' / reorder '.$row['reorder_point']:'').' — used by '.$row['menu_source_count'].' menu item(s), '.$row['recipe_source_count'].' recipe(s)';
    return ['skill'=>$low?'inventory.low_stock':'inventory.search','answer'=>($low?'Inventory requiring attention:':'Matching inventory:')."\n- ".implode("\n- ",$lines),'data'=>$rows,'sources'=>array_column($rows,'public_id')];
}

function operations_agent_task_answer(PDO $pdo,int $organizationId,string $message,?int $userId=null): array
{
    $normalized=mb_strtolower($message,'UTF-8');$category=preg_match('/\bprep\b/u',$normalized)?'prep':'';$my=preg_match('/\b(my|assigned to me)\b/u',$normalized)?$userId:null;$rows=operations_task_list($pdo,$organizationId,['category'=>$category,'userId'=>$my,'query'=>'']);
    if(!$rows)return ['skill'=>'tasks.list','answer'=>$category?'The prep list is currently empty.':'No matching restaurant tasks are open.','data'=>[],'sources'=>[]];
    $lines=[];foreach(array_slice($rows,0,30) as $row)$lines[]=$row['title'].' — '.($row['category_name']?:'General').' — '.$row['status'].' — '.($row['assigned_names']?:'unassigned').($row['due_at']?' — due '.$row['due_at']:'');
    return ['skill'=>$category?'tasks.prep_list':'tasks.list','answer'=>($category?'Current prep list:':'Restaurant tasks:')."\n- ".implode("\n- ",$lines),'data'=>$rows,'sources'=>array_column($rows,'public_id')];
}
