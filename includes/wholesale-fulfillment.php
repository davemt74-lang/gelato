...
{
    $order=wholesale_fulfillment_order($pdo,$org,$orderPublic);
    $locationsQ=$pdo->prepare("SELECT public_id,name,address_line1,address_line2,city,state_region,postal_code,country_code,contact_name,contact_phone,delivery_notes,is_primary FROM wholesale_account_locations WHERE organization_id=? AND wholesale_account_id=? AND status='active' ORDER BY is_primary DESC,name");
