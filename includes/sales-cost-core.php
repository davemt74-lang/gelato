...
{
    if(!purchasing_ready($pdo))return [];$q=$pdo->prepare("SELECT h.inventory_item_id,h.vendor_id,i.name,h.price_per_pack,h.pack_size,h.unit,h.effective_at,v.name vendor_name FROM vendor_price_history h JOIN inventory_items i ON i.id=h.inventory_item_id AND i.organization_id=h.organization_id JOIN vendors v ON v.id=h.vendor_id AND v.organization_id=h.organization_id WHERE h.organization_id=? AND h.effective_at<? ORDER BY h.inventory_item_id,h.vendor_id,h.effective_at DESC,h.id DESC LIMIT 2000");$q->execute([$org,(new DateTimeImmutable($to))->modify('+1 day')->format('Y-m-d 00:00:00')]);$seen=[];$out=[];foreach($q->fetchAll() as $r){$key=(int)$r['inventory_item_id'].'|'.(int)$r['vendor_id'];if(!isset($seen[$key])){$seen[$key]=[$r];continue;}if(count($seen[$key])===1){$seen[$key][]=$r;$latest=$seen[$key][0];$prior=$r;$latestUnit=(float)$latest['pack_size']>0?(float)$latest['price_per_pack']/(float)$latest['pack_size']:null;$priorUnit=(float)$prior['pack_size']>0?(float)$prior['price_per_pack']/(float)$prior['pack_size']:null;if($latestUnit!==null&&$priorUnit!==null&&$priorUnit>0){$out[]=['inventoryItem'=>$latest['name'],'vendor'=>$latest['vendor_name'],'latestUnitCost'=>round($latestUnit,4),'priorUnitCost'=>round($priorUnit,4),'changePercent'=>round(($latestUnit-$priorUnit)/$priorUnit*100,2),'latestAt'=>$latest['effective_at'],'priorAt'=>$prior['effective_at']];}}}
