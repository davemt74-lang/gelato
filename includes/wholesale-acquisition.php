...
function wholesale_acquisition_completion(array $input): int
{
    ...
    $filled=0;
    foreach($keys as $key)if(trim((string)($input[$key]??''))!=='')$filled++;
...
function wholesale_acquisition_validate_assignee(PDO $pdo,int $org,mixed $value): ?int
{
    ...
    $q=$pdo->prepare("SELECT COUNT(*) FROM users u JOIN organization_memberships om ON om.user_id=u.id AND om.organization_id=? WHERE u.id=? AND u.status='active' AND u.archived_at IS NULL");
    $q->execute([$org,$id]);
...
function wholesale_acquisition_save(PDO $pdo,int $org,array $input,int $userId,bool $complete=false): array
{
    ...
    $business=mb_substr(trim((string)($input['businessName']??'')),0,200,'UTF-8');
    ...
    if($website!==''&&(!filter_var($website,FILTER_VALIDATE_URL)||strlen($website)>500))throw new InvalidArgumentException('Enter a valid website URL including https://.');

    $public=trim((string)($input['id']??''));
    $qualification=(string)($input['qualificationStatus']??'prospect');
    if(!in_array($qualification,['prospect','qualified','sample','quoted','not_fit'],true))$qualification='prospect';
    $stage=$complete?wholesale_acquisition_stage($qualification):'new';
    $prob=wholesale_acquisition_probability($stage);
    $packages=array_values(array_unique(array_filter(array_map(static fn($v)=>mb_substr(trim((string)$v),0,80,'UTF-8'),(array)($input['packageSizes']??[])))));
    $assigned=wholesale_acquisition_validate_assignee($pdo,$org,$input['assignedTo']??null);

    $follow=trim((string)($input['nextFollowupAt']??''));
    if($follow!==''){
        $d=DateTimeImmutable::createFromFormat('Y-m-d\TH:i',$follow);
        if(!$d)$d=DateTimeImmutable::createFromFormat('Y-m-d H:i:s',$follow);
        if(!$d)throw new InvalidArgumentException('Next follow-up date is invalid.');
        $follow=$d->format('Y-m-d H:i:s');
    }else $follow=null;

    $start=trim((string)($input['desiredStartDate']??''));
    if($start!==''){
        $d=DateTimeImmutable::createFromFormat('Y-m-d',$start);
        if(!$d||$d->format('Y-m-d')!==$start)throw new InvalidArgumentException('Desired start date is invalid.');
    }
    $estimatedValue=isset($input['estimatedValue'])&&$input['estimatedValue']!==''?max(0,(float)$input['estimatedValue']):null;
    $completion=wholesale_acquisition_completion($input);
    ...
    try{
        if($public===''){
            ...
            $q->execute([
                $org,$public,$business,$contact,$email,
                mb_substr(trim((string)($input['phone']??'')),0,50,'UTF-8')?:null,$website?:null,
                mb_substr(trim((string)($input['businessType']??'')),0,80,'UTF-8')?:null,
                mb_substr(trim((string)($input['location']??'')),0,300,'UTF-8')?:null,
                mb_substr(trim((string)($input['estimatedMonthlyVolume']??'')),0,120,'UTF-8')?:null,
                mb_substr(trim((string)($input['orderFrequency']??'')),0,120,'UTF-8')?:null,
                json_encode($packages,JSON_THROW_ON_ERROR),
                mb_substr(trim((string)($input['flavorsInterest']??'')),0,10000,'UTF-8')?:null,
                !empty($input['privateLabelInterest'])?1:0,
                mb_substr(trim((string)($input['freezerCapacity']??'')),0,120,'UTF-8')?:null,
                mb_substr(trim((string)($input['fulfillmentPreference']??'')),0,80,'UTF-8')?:null,
                $start?:null,
                mb_substr(trim((string)($input['currentSupplier']??'')),0,200,'UTF-8')?:null,
                mb_substr(trim((string)($input['notes']??'')),0,10000,'UTF-8')?:null,
                $stage,$estimatedValue,$prob,$assigned,$follow
            ]);
            $leadId=(int)$pdo->lastInsertId();
        }else{
            $q=$pdo->prepare('SELECT * FROM wholesale_leads WHERE organization_id=? AND public_id=? AND archived_at IS NULL FOR UPDATE');
            $q->execute([$org,$public]);$lead=$q->fetch();
            if(!$lead)throw new InvalidArgumentException('Wholesale lead not found.');
            $leadId=(int)$lead['id'];
            if(!$complete){
                $stage=(string)$lead['pipeline_stage'];
                $prob=(int)$lead['probability_percent'];
            }
            ...
            $q->execute([
                ...
                mb_substr(trim((string)($input['currentSupplier']??'')),0,200,'UTF-8')?:null,
                mb_substr(trim((string)($input['notes']??'')),0,10000,'UTF-8')?:null,
                $stage,$estimatedValue,$prob,$assigned,$follow,$org,$leadId
            ]);
        }
        ...
        $q->execute([
            $org,$leadId,
            mb_substr(trim((string)($input['acquisitionChannel']??'')),0,100,'UTF-8')?:null,$qualification,
            mb_substr(trim((string)($input['buyerRole']??'')),0,120,'UTF-8')?:null,
            isset($input['locationCount'])&&$input['locationCount']!==''?max(0,(int)$input['locationCount']):null,
            isset($input['estimatedMonthlyUnits'])&&$input['estimatedMonthlyUnits']!==''?max(0,(float)$input['estimatedMonthlyUnits']):null,
            isset($input['estimatedMonthlyRevenue'])&&$input['estimatedMonthlyRevenue']!==''?max(0,(float)$input['estimatedMonthlyRevenue']):null,
            mb_substr(trim((string)($input['sampleInterest']??'')),0,120,'UTF-8')?:null,
            mb_substr(trim((string)($input['decisionTimeline']??'')),0,160,'UTF-8')?:null,
            mb_substr(trim((string)($input['painPoints']??'')),0,10000,'UTF-8')?:null,
            mb_substr(trim((string)($input['buyingProcess']??'')),0,10000,'UTF-8')?:null,
            mb_substr(trim((string)($input['storageNotes']??'')),0,10000,'UTF-8')?:null,
            mb_substr(trim((string)($input['pricingNotes']??'')),0,10000,'UTF-8')?:null,
            mb_substr(trim((string)($input['nextStep']??'')),0,10000,'UTF-8')?:null,
            $completion,$complete?1:0,$userId,$userId
        ]);
        ...
        $activity->execute([
            $org,$leadId,$complete?'Acquisition worksheet completed':'Acquisition worksheet saved',
            $complete?'Qualification: '.$qualification.'. Pipeline stage: '.$stage.'.':'Draft acquisition worksheet updated without changing pipeline stage.',
            json_encode(['completionPercent'=>$completion,'qualificationStatus'=>$qualification,'stage'=>$stage],JSON_THROW_ON_ERROR),$userId
        ]);
        $pdo->commit();
...
function wholesale_acquisition_get(PDO $pdo,int $org,string $public): ?array
{
    ...
    $q->execute([$org,$public]);$row=$q->fetch();
    if(!$row)return null;
    $row['package_sizes']=json_decode((string)($row['package_sizes_json']??'[]'),true)?:[];
    unset($row['package_sizes_json']);
    return $row;
}
