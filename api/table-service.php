...
try{
    ...
    if($action==='table.transfer'){$check=service_reservation_protection_transfer($pdo,$org,$checkPublic,(string)($input['destinationTablePublicId']??''),$uid);app_audit($pdo,$org,$uid,'table_service.table_transfer','pos_check',$checkPublic,null,['destinationTablePublicId'=>(string)($input['destinationTablePublicId']??'')]);app_json_response(['ok'=>true,'check'=>$check,'map'=>service_ops_canonical_map($pdo,$org,$locationId),'openChecks'=>service_visit_open_checks($pdo,$org,$locationId)]);}
