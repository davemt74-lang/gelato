...
if(!table_service_ready($pdo))app_json_response(['ok'=>false,'message'=>'Table Service migration is not installed. Run upgrade.php.'],503);
...
try{
    ...
    if(str_starts_with($action,'section.')||$action==='table.save'){
        ...
        if($action==='section.save'){$section=table_service_section_save($pdo,$org,$locationId,$input,$uid);app_audit($pdo,$org,$uid,'table_service.section_saved','service_section',(string)$section['publicId'],null,['locationId'=>$locationId,'name'=>$section['name']]);app_json_response(['ok'=>true,'section'=>$section,'map'=>table_service_map($pdo,$org,$locationId)]);}
        if($action==='section.assign'){$assignment=table_service_section_assign($pdo,$org,$locationId,(string)($input['sectionPublicId']??''),isset($input['assignedUserId'])&&$input['assignedUserId']!==null?(int)$input['assignedUserId']:null,(string)($input['businessDate']??date('Y-m-d')),$uid);app_audit($pdo,$org,$uid,'table_service.section_assigned','service_section',(string)($input['sectionPublicId']??''),null,$assignment);app_json_response(['ok'=>true,'assignment'=>$assignment,'map'=>table_service_map($pdo,$org,$locationId)]);}
        if($action==='table.save'){$table=table_service_table_save($pdo,$org,$locationId,$input,$uid);app_audit($pdo,$org,$uid,'table_service.table_saved','service_table',(string)$table['publicId'],null,['locationId'=>$locationId,'name'=>$table['name'],'capacity'=>$table['capacity']]);app_json_response(['ok'=>true,'table'=>$table,'map'=>table_service_map($pdo,$org,$locationId)]);}
    }
    ...
    if($action==='party.seat'){$check=table_service_seat($pdo,$org,$locationId,(string)($input['tablePublicId']??''),(int)($input['partySize']??1),isset($input['serverUserId'])&&$input['serverUserId']!==null?(int)$input['serverUserId']:null,(string)($input['notes']??''),$uid);app_audit($pdo,$org,$uid,'table_service.party_seated','pos_check',(string)$check['publicId'],null,['tablePublicId'=>(string)($input['tablePublicId']??''),'partySize'=>(int)($input['partySize']??1)]);app_json_response(['ok'=>true,'check'=>$check,'map'=>table_service_map($pdo,$org,$locationId),'openChecks'=>pos_open_checks($pdo,$org,$locationId)],201);}
    if($action==='table.state'){$map=table_service_table_state($pdo,$org,(string)($input['tablePublicId']??''),(string)($input['state']??''),$uid);app_audit($pdo,$org,$uid,'table_service.table_state','service_table',(string)($input['tablePublicId']??''),null,['state'=>(string)($input['state']??'')]);app_json_response(['ok'=>true,'map'=>$map]);}
    if($checkPublic==='')throw new InvalidArgumentException('Choose an open table-service check.');
    if($action==='server.assign'){$check=table_service_assign_server($pdo,$org,$checkPublic,(int)($input['serverUserId']??0),$uid);app_audit($pdo,$org,$uid,'table_service.server_assigned','pos_check',$checkPublic,null,['serverUserId'=>(int)($input['serverUserId']??0)]);app_json_response(['ok'=>true,'check'=>$check,'map'=>table_service_map($pdo,$org,$locationId)]);}
    ...
    if($action==='table.transfer'){$check=table_service_transfer($pdo,$org,$checkPublic,(string)($input['destinationTablePublicId']??''),$uid);app_audit($pdo,$org,$uid,'table_service.table_transfer','pos_check',$checkPublic,null,['destinationTablePublicId'=>(string)($input['destinationTablePublicId']??'')]);app_json_response(['ok'=>true,'check'=>$check,'map'=>table_service_map($pdo,$org,$locationId),'openChecks'=>pos_open_checks($pdo,$org,$locationId)]);}
    if($action==='check.split'){$result=table_service_split($pdo,$org,$checkPublic,is_array($input['itemIds']??null)?$input['itemIds']:[],$uid);app_audit($pdo,$org,$uid,'table_service.check_split','pos_check',$checkPublic,null,['createdCheckPublicId'=>$result['created']['publicId'],'itemIds'=>$input['itemIds']??[]]);app_json_response(['ok'=>true]+$result+['map'=>table_service_map($pdo,$org,$locationId),'openChecks'=>pos_open_checks($pdo,$org,$locationId)],201);}
    if($action==='check.merge'){$target=trim((string)($input['targetCheckPublicId']??''));$check=table_service_merge($pdo,$org,$checkPublic,$target,$uid);app_audit($pdo,$org,$uid,'table_service.check_merged','pos_check',$target,null,['sourceCheckPublicId'=>$checkPublic]);app_json_response(['ok'=>true,'check'=>$check,'map'=>table_service_map($pdo,$org,$locationId),'openChecks'=>pos_open_checks($pdo,$org,$locationId)]);}
