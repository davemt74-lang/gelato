...
try{
    if($_SERVER['REQUEST_METHOD']==='GET'){
        ...
        $payload=['ok'=>true,'locationId'=>$locationId,'locations'=>pos_locations($pdo,$org),'businessDate'=>service_ops_business_date($pdo,$org,$locationId),'map'=>service_ops_map($pdo,$org,$locationId),'menu'=>pos_menu($pdo,$org),'openChecks'=>pos_open_checks($pdo,$org,$locationId),'permissions'=>['use'=>$canUse,'manage'=>$canManage]];
    ...
    if(str_starts_with($action,'section.')||$action==='table.save'){
        ...
        if($action==='table.save'){
            ...
            $table=table_service_table_save($pdo,$org,$locationId,$input,$uid);app_audit($pdo,$org,$uid,'table_service.table_saved','service_table',(string)$table['publicId'],null,['locationId'=>$locationId,'name'=>$table['name'],'capacity'=>$table['capacity']]);app_json_response(['ok'=>true,'table'=>$table,'map'=>service_ops_map($pdo,$org,$locationId)]);
        }
    }
    ...
    if($action==='server.assign'){$check=table_service_assign_server($pdo,$org,$checkPublic,(int)($input['serverUserId']??0),$uid);app_audit($pdo,$org,$uid,'table_service.server_assigned','pos_check',$checkPublic,null,['serverUserId'=>(int)($input['serverUserId']??0)]);app_json_response(['ok'=>true,'check'=>$check,'map'=>service_ops_map($pdo,$org,$locationId)]);}
    ...
    if($action==='table.transfer'){$check=service_ops_transfer($pdo,$org,$checkPublic,(string)($input['destinationTablePublicId']??''),$uid);app_audit($pdo,$org,$uid,'table_service.table_transfer','pos_check',$checkPublic,null,['destinationTablePublicId'=>(string)($input['destinationTablePublicId']??'')]);app_json_response(['ok'=>true,'check'=>$check,'map'=>service_ops_map($pdo,$org,$locationId),'openChecks'=>pos_open_checks($pdo,$org,$locationId)]);}
    if($action==='check.split'){$movedGuests=isset($input['movedGuestCount'])&&$input['movedGuestCount']!==''?(int)$input['movedGuestCount']:null;$result=service_ops_split($pdo,$org,$checkPublic,is_array($input['itemIds']??null)?$input['itemIds']:[],$uid,$movedGuests);app_audit($pdo,$org,$uid,'table_service.check_split','pos_check',$checkPublic,null,['createdCheckPublicId'=>$result['created']['publicId'],'itemIds'=>$input['itemIds']??[],'movedGuestCount'=>$result['created']['guestCount']]);app_json_response(['ok'=>true]+$result+['map'=>service_ops_map($pdo,$org,$locationId),'openChecks'=>pos_open_checks($pdo,$org,$locationId)],201);}
