...
try{
    ...
    if($action==='party.seat'){$check=service_visit_seat($pdo,$org,$locationId,(string)($input['tablePublicId']??''),(int)($input['partySize']??1),isset($input['serverUserId'])&&$input['serverUserId']!==null?(int)$input['serverUserId']:null,(string)($input['notes']??''),$uid);app_audit($pdo,$org,$uid,'table_service.party_seated','pos_check',(string)$check['publicId'],null,['tablePublicId'=>(string)($input['tablePublicId']??''),'partySize'=>(int)($input['partySize']??1)]);app_json_response(['ok'=>true,'check'=>$check,'map'=>service_ops_canonical_map($pdo,$org,$locationId),'openChecks'=>service_visit_open_checks($pdo,$org,$locationId)],201);}
