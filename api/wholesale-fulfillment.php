...
if($_SERVER['REQUEST_METHOD']==='GET'){
    ...
    try{
        ...
        if($action==='detail'){
            ...
            app_json_response(['ok'=>true,'detail'=>wholesale_fulfillment_order_detail($pdo,$org,$id)]);
        }
        if($action==='batch'){
...
try{
    if($action==='plan.save'){
        ...
        $detail=wholesale_fulfillment_save_order_plan($pdo,$org,$orderId,$input,$userId);
        operations_sync_wholesale_tasks($pdo,$org,$userId);
        app_json_response(['ok'=>true,'message'=>'Fulfillment location and windows saved.','detail'=>$detail]);
    }
    if($action==='batch.create'){
        $orderId=trim((string)($input['orderId']??''));if($orderId==='')throw new InvalidArgumentException('Wholesale order is required.');
        $batch=wholesale_fulfillment_create_batch($pdo,$org,$orderId,$input,$userId);
        operations_sync_wholesale_tasks($pdo,$org,$userId);
        app_json_response(['ok'=>true,'message'=>'Fulfillment batch created.','batch'=>$batch,'detail'=>wholesale_fulfillment_order_detail($pdo,$org,$orderId)]);
    }
    ...
    if($action==='batch.ready'||$action==='batch.dispatch'||$action==='batch.cancel'){
        ...
        operations_sync_wholesale_tasks($pdo,$org,$userId);
        app_json_response(['ok'=>true,'message'=>'Fulfillment batch moved to '.$next.'.','batch'=>$batch,'detail'=>wholesale_fulfillment_order_detail($pdo,$org,(string)$batch['orderId'])]);
    }
    if($action==='batch.deliver'){
        ...
        operations_sync_wholesale_tasks($pdo,$org,$userId);
        app_json_response(['ok'=>true,'message'=>'Fulfillment delivered and canonical inventory consumed.','batch'=>$batch,'detail'=>wholesale_fulfillment_order_detail($pdo,$org,(string)$batch['orderId'])]);
    }
    throw new InvalidArgumentException('Unsupported Wholesale fulfillment action.');
}catch(InvalidArgumentException|RuntimeException $e){app_json_response(['ok'=>false,'message'=>$e->getMessage()],422);}
