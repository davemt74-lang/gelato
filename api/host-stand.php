...
try{
    ...
    if($action==='reservation.status'){$reservation=service_ops_reservation_status($pdo,$org,$public,(string)($input['status']??''),$uid);app_audit($pdo,$org,$uid,'host.reservation_status','guest_reservation',$public,null,['status'=>$reservation['status']]);app_json_response(['ok'=>true,'reservation'=>$reservation,'dashboard'=>host_dashboard($pdo,$org,$locationId,$date,$uid)]);}
    if($action==='reservation.assign'){$reservation=service_ops_reservation_assign($pdo,$org,$public,(array)($input['tablePublicIds']??[]),$uid);app_audit($pdo,$org,$uid,'host.reservation_tables','guest_reservation',$public,null,['tables'=>array_column($reservation['tables'],'publicId')]);app_json_response(['ok'=>true,'reservation'=>$reservation,'dashboard'=>host_dashboard($pdo,$org,$locationId,$date,$uid)]);}
