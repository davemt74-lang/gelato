...
try{
    if($_SERVER['REQUEST_METHOD']==='GET'){
        $locationId=host_api_location($pdo,$org,$membership);$date=host_api_date($pdo,$org,$locationId,(string)($_GET['date']??service_ops_business_date($pdo,$org,$locationId)));host_api_reconcile($pdo,$org,$locationId,$uid);$payload=['ok'=>true,'locations'=>pos_locations($pdo,$org),'date'=>$date,'permissions'=>['use'=>$canUse,'manage'=>$canManage],'dashboard'=>service_visit_dashboard($pdo,$org,$locationId,$date,$uid)];
        $at=trim((string)($_GET['availabilityAt']??''));if($at!=='')$payload['availability']=service_ops_availability($pdo,$org,$locationId,$at,(int)($_GET['partySize']??2),(int)($_GET['durationMinutes']??90));
