...
try {
    ...
    if (!$user) {
        ...
        echo "window.MENU_SECTIONS=[];window.MENU_NOTES=[];window.ALLERGEN_KNOWLEDGE={majorAllergens:[],directRules:[],verifyRules:[],alwaysVerify:[]};window.MENU_SOURCE_META={ok:false,message:'Authentication required'};\n";
        exit;
    }
    $pdo = app_pdo();
    ...
    $allergens = menu_training_allergen_knowledge();
    $meta = [
        'ok' => true,
        'source' => 'database',
        ...
        'trainingKnowledge' => ['menu'=>true,'ingredients'=>true,'notes'=>true,'allergens'=>true],
    ];
    ...
    echo 'window.ALLERGEN_KNOWLEDGE=' . json_encode($allergens, $flags) . ";\n";
    echo 'window.MENU_SOURCE_META=' . json_encode($meta, $flags) . ";\n";
} catch (Throwable $exception) {
    error_log('Menu knowledge endpoint error: ' . $exception->getMessage());
    http_response_code(500);
    echo "window.MENU_SECTIONS=[];window.MENU_NOTES=[];window.ALLERGEN_KNOWLEDGE={majorAllergens:[],directRules:[],verifyRules:[],alwaysVerify:[]};window.MENU_SOURCE_META={ok:false,message:'Menu data is temporarily unavailable.'};\n";
}
