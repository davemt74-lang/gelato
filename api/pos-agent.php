...
if($action!=='ask')app_json_response(['ok'=>false,'message'=>'Unsupported POS Agent action.'],422);
...
function pos_agent_customer_access(PDO $pdo,array $user,int $locationId): bool
{
...
function pos_agent_item_map(array $items): array
{
    $map=[];foreach($items as $item)$map[(int)$item['id']]=$item;return $map;
}
function pos_agent_cart_item_ids(array $check): array
{
    $ids=[];foreach($check['items']??[] as $line){if((string)($line['status']??'active')!=='active')continue;$id=(int)($line['menu_item_id']??0);if($id>0)$ids[]=$id;}return array_values(array_unique($ids));
}
...
function pos_agent_pick_items(array $knowledge,array $check,string $message,int $focusedLineId=0): array
{
    ...
    if($focusedLineId>0){foreach($check['items']??[] as $line){if((int)$line['id']!==$focusedLineId||(string)($line['status']??'active')!=='active')continue;$menuItemId=(int)($line['menu_item_id']??0);foreach($knowledge as $item)if((int)$item['id']===$menuItemId)return [$item];}}
    return count($knowledge)===1?$knowledge:$knowledge;
}
function pos_agent_item_sources(array $items): array
{
    $sources=[];foreach($items as $item)$sources[]='Menu Knowledge → '.(string)$item['name'];
...
{
    $lines=pos_agent_lines_for_menu_item($check,(int)$item['id']);$notes=[];
    foreach($lines as $line){$text=(string)($line['option_name_snapshot']??'');if(trim((string)($line['special_instructions']??''))!=='')$text.=' — guest note: '.trim((string)$line['special_instructions']);if($text!=='')$notes[]=$text;}
...
function pos_agent_allergen_answer(array $items,array $check): array
{
    ...
    foreach($items as $item){
        ...
        $guestNotes=pos_agent_item_lines_text($check,$item);if($guestNotes!=='')$text.=' Current check: '.$guestNotes.'.';
        $lines[]=$text;
    }
    $answer=($lines?implode("\n- ",array_map(static fn(string $v):string=>$v,$lines)):'No active cart item matched that question.');
    if($lines)$answer="Allergen training profile for the current check:\n- ".$answer;
...
{
    $lines=[];foreach($items as $item){$ingredients=$item['ingredients']??[];$text=(string)$item['name'].': '.($ingredients?implode(', ',$ingredients):'no complete ingredient list is recorded in the current training data');$guestNotes=pos_agent_item_lines_text($check,$item);if($guestNotes!=='')$text.='. Current check: '.$guestNotes;$lines[]=$text;}
    return ['answer'=>$lines?"Recorded Ingredient Catalog entries:\n- ".implode("\n- ",$lines):'I could not match an active cart item to the Ingredient Catalog.','sources'=>array_values(array_unique([...pos_agent_item_sources($items),'Ingredient Catalog']))];
}
function pos_agent_prep_answer(array $items,array $check): array
{
    $lines=[];foreach($items as $item){$parts=[];$prep=trim((string)($item['preparationNotes']??''));$section=trim((string)($item['sectionNote']??''));if($prep!=='')$parts[]='prep note: '.$prep;if($section!=='')$parts[]='service/menu note: '.$section;$guestNotes=pos_agent_item_lines_text($check,$item);if($guestNotes!=='')$parts[]='current check: '.$guestNotes;if(!$parts)$parts[]='no item-specific cooking/preparation note is documented in the current training data';$lines[]=(string)$item['name'].': '.implode(' · ',$parts);}
    $answer=$lines?"Current Menu Notes / preparation guidance:\n- ".implode("\n- ",$lines):'I could not match an active cart item to the current menu training knowledge.';
...
{
    $lines=[];foreach($items as $item){$text=(string)$item['name'].': '.pos_agent_item_price_text($item);$guestNotes=pos_agent_item_lines_text($check,$item);if($guestNotes!=='')$text.=' · current check '.$guestNotes;$lines[]=$text;}
...
{
    $lines=[];foreach($check['items']??[] as $line){if((string)($line['status']??'active')!=='active')continue;$entry=rtrim(rtrim(number_format((float)$line['quantity'],2,'.',''),'0'),'.').'× '.(string)$line['item_name_snapshot'];if(trim((string)($line['option_name_snapshot']??''))!=='')$entry.=' — '.(string)$line['option_name_snapshot'];if(trim((string)($line['special_instructions']??''))!=='')$entry.=' — note: '.trim((string)$line['special_instructions']);$lines[]=$entry;}
    $customer=$check['customer']['displayName']??null;
    $answer=(string)$check['checkNumber'].' · '.str_replace('_',' ',(string)$check['serviceMode']).' · '.(int)$check['guestCount'].' guest'.((int)$check['guestCount']===1?'':'s').'.';
    $answer.=' Customer: '.($customer?(string)$customer:'walk-in / no CRM customer attached').'.';
...
function pos_agent_customer_context(PDO $pdo,int $org,array $user,array $check): ?array
{
    ...
    $locationId=(int)$check['locationId'];if(!pos_agent_customer_access($pdo,$user,$locationId))return ['identity'=>['displayName'=>(string)$customer['displayName']],'limited'=>true];
    $row=crm_customer_row($pdo,$org,(string)$customer['publicId']);$metrics=crm_metrics($pdo,$org,(int)$row['id']);
    $promotions=[];
    if(customer_inbox_ready($pdo)){
        foreach(customer_inbox_messages($pdo,$org,(int)$row['id'],40) as $message){
            if(!in_array((string)$message['type'],['promotion','reward'],true))continue;
            $locationName=trim((string)($message['locationName']??''));
            if($locationName!==''&&$locationName!==(string)$check['locationName'])continue;
            $promotions[]=$message;if(count($promotions)>=12)break;
        }
    }
    return [
        'identity'=>['publicId'=>(string)$row['public_id'],'displayName'=>(string)$row['display_name']],
        'limited'=>false,
        'metrics'=>[
            'visits'=>(int)$metrics['visits'],'lifetimeSpend'=>(float)$metrics['lifetimeSpend'],'averageCheck'=>(float)$metrics['averageCheck'],
            'firstVisit'=>$metrics['firstVisit'],'lastVisit'=>$metrics['lastVisit'],
            'recentChecks'=>array_slice($metrics['recentChecks']??[],0,5),'favoriteItems'=>array_slice($metrics['favoriteItems']??[],0,8),
        ],
        'promotions'=>$promotions,
    ];
}
function pos_agent_customer_answer(?array $customerContext): array
{
    if(!$customerContext)return ['answer'=>'This check is currently a walk-in / unlinked guest, so there is no repeat-customer history to display.','sources'=>['POS check']];
    $name=(string)($customerContext['identity']['displayName']??'Customer');
    if(!empty($customerContext['limited']))return ['answer'=>$name.' is attached to this check. Additional customer history is not shown because this staff account does not have CRM customer-view/link access for the transaction.','sources'=>['POS customer link']];
    $m=$customerContext['metrics'];$visits=(int)$m['visits'];
    $answer=$name.' · '.($visits>1?'repeat customer':($visits===1?'one completed prior visit':'no completed prior visit yet')).'. '.$visits.' paid visit'.($visits===1?'':'s').' · average check '.pos_agent_money((float)$m['averageCheck']).' · lifetime spend '.pos_agent_money((float)$m['lifetimeSpend']).'.';
...
function pos_agent_recent_orders_answer(?array $customerContext): array
{
    if(!$customerContext)return ['answer'=>'No CRM customer is attached to this check, so there is no customer order history to show.','sources'=>['POS check']];
    if(!empty($customerContext['limited']))return ['answer'=>'A customer is attached, but this staff account does not have access to the customer’s order history.','sources'=>['POS customer link']];
    $rows=$customerContext['metrics']['recentChecks']??[];if(!$rows)return ['answer'=>'This customer does not have a completed paid POS order in the current CRM history yet.','sources'=>['Customer CRM']];
...
function pos_agent_promotions_answer(?array $customerContext): array
{
    if(!$customerContext)return ['answer'=>'No CRM customer is attached to this check, so customer-specific promotions or rewards cannot be matched yet.','sources'=>['POS check']];
    if(!empty($customerContext['limited']))return ['answer'=>'A customer is attached, but this staff account does not have access to customer-specific promotion history.','sources'=>['POS customer link']];
    $rows=$customerContext['promotions']??[];if(!$rows)return ['answer'=>'There are no active, undismissed customer Inbox promotions or rewards for this customer at this location right now.','sources'=>['Customer Inbox Promotions']];
...
try{
    $checkPublicId=pos_agent_clean_id($pageContext['checkPublicId']??'');
    $check=null;$locationId=max(0,(int)($pageContext['locationId']??0));
    if($checkPublicId!==''){
        ...
        $check=pos_check_details($pdo,$org,$checkPublicId);
        $check['customer']=crm_ready($pdo)?crm_check_customer($pdo,$org,$checkPublicId):null;
    ...
    $focusedLineId=max(0,(int)($pageContext['focusedLineId']??0));
    $selected=$check?pos_agent_pick_items($cartKnowledge,$check,$message,$focusedLineId):[];
    if(!$selected){$selected=menu_training_search_items($pdo,$org,$message,12);}
    $lower=mb_strtolower($message,'UTF-8');
    $customerContext=$check?pos_agent_customer_context($pdo,$org,$user,$check):null;
    ...
    $skill='pos.context';
    if(str_contains(implode(' ',array_map('strval',$result['sources']??[])),'Allergen'))$skill='pos.menu_allergens';
    elseif(str_contains(implode(' ',array_map('strval',$result['sources']??[])),'Ingredient'))$skill='pos.menu_ingredients';
    elseif(str_contains(implode(' ',array_map('strval',$result['sources']??[])),'Customer Inbox'))$skill='pos.customer_promotions';
    elseif(str_contains(implode(' ',array_map('strval',$result['sources']??[])),'Customer CRM'))$skill='pos.customer_context';
    elseif(str_contains(implode(' ',array_map('strval',$result['sources']??[])),'Menu Notes'))$skill='pos.menu_notes';
    elseif(str_contains(implode(' ',array_map('strval',$result['sources']??[])),'Menu Knowledge'))$skill='pos.menu_knowledge';

    app_audit($pdo,$org,$uid,'agent.pos_context_used','pos_check',$checkPublicId?:null,null,[
        'skill'=>$skill,'locationId'=>$locationId?:null,'checkPublicId'=>$checkPublicId?:null,
        'customerLinked'=>!empty($check['customer']['publicId']),'cartMenuItems'=>count($cartKnowledge),
        'message'=>mb_substr($message,0,300,'UTF-8'),
    ]);
    app_json_response(['ok'=>true,'skill'=>$skill,'answer'=>$result['answer'],'data'=>[
        'checkPublicId'=>$checkPublicId?:null,'locationId'=>$locationId?:null,
        'cartMenuItemIds'=>array_column($cartKnowledge,'id'),'customerLinked'=>!empty($check['customer']['publicId']),
    ],'sources'=>$result['sources']??[]]);
}catch(Throwable $e){
    $status=$e instanceof InvalidArgumentException?422:($e instanceof DomainException?403:500);
    if($status===500)error_log('POS Agent error: '.$e->getMessage());
