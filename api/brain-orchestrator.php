...
try{
    ...
    $snapshot=live_shift_merge_brain_snapshot($pdo,$user,agent_brain_orchestration_snapshot($pdo,$user,null));$result=agent_brain_orchestration_answer($snapshot,$message);
    app_audit($pdo,(int)$user['organization_id'],(int)$user['id'],'agent.brain_orchestration_used','agent_brain','manager',null,['signalCount'=>count($snapshot['signals']),'nextMoveCount'=>count($snapshot['nextMoves']),'liveShiftIntegrated'=>true]);
