...
function placement_asset_payload(array $row): array
{
    return [
        ...
        'placedAt' => $row['floor_plan_placed_at'] ?? null,
...
function placement_sync_brain(PDO $pdo, int $organizationId, int $assetId, ?int $userId): void
{
...
if (!placement_table_ready($pdo)) {
    app_json_response([
        'ok' => false,
        'message' => 'Canonical floor-plan equipment migration is not installed. Import database/20260912_canonical_floor_equipment.sql first.',
    ], 503);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    app_json_response([
        ...
        'plan' => [
            ...
            'widthFt' => (float)$plan['width_ft'],
            'depthFt' => (float)$plan['depth_ft'],
            'scale' => (float)$plan['scale_px_per_ft'],
...
if ($assetPublicId === '') {
    app_json_response(['ok' => false, 'message' => 'An equipment asset id is required.'], 422);
}
...
try {
    ...
    if (!$asset) {
        throw new DomainException('Equipment asset not found.');
    }

    if ($action === 'place' || $action === 'move') {
        $planId = placement_clean_id($input['planId'] ?? '');
        if ($planId === '' || !placement_plan($pdo, $organizationId, $planId)) {
            throw new DomainException('Floor plan not found.');
        }
        $plan = placement_plan($pdo, $organizationId, $planId);
        ...
        $statement = $pdo->prepare(
            "UPDATE equipment_assets
             SET floor_plan_public_id=NULL, floor_plan_component_id=NULL, floor_plan_x_ft=NULL, floor_plan_y_ft=NULL,
                 floor_plan_rotation_deg=0, floor_plan_z_index=1, floor_plan_locked=0, floor_plan_placed_at=NULL,
                 updated_by=?, updated_at=NOW(6)
             WHERE id=? AND organization_id=?"
        );
        ...
        $statement = $pdo->prepare(
            "UPDATE equipment_assets SET width_inches=?, depth_inches=?, updated_by=?, updated_at=NOW(6)
             WHERE id=? AND organization_id=?"
        );
        ...
        $statement = $pdo->prepare(
            "UPDATE equipment_assets SET floor_plan_locked=?, updated_by=?, updated_at=NOW(6)
             WHERE id=? AND organization_id=?"
        );
    ...
    placement_sync_brain($pdo, $organizationId, (int)$asset['id'], $userId);
    app_audit(
        $pdo,
        $organizationId,
        $userId,
        ...
        $assetPublicId,
        [
            'floorPlanId'=>$asset['floor_plan_public_id'] ?? null,
            'xFt'=>$asset['floor_plan_x_ft'] ?? null,
            'yFt'=>$asset['floor_plan_y_ft'] ?? null,
            'rotationDeg'=>$asset['floor_plan_rotation_deg'] ?? 0,
            'widthInches'=>$asset['width_inches'] ?? null,
            'depthInches'=>$asset['depth_inches'] ?? null,
        ],
        $auditNew
    );
    $pdo->commit();
