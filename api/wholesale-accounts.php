...
if($action==='quote'){
    if(wholesale_commerce_ready($pdo)){
        try{$created=wholesale_commerce_create_quote($pdo,$organizationId,$account,$input,(int)$user['id']);}catch(Throwable $e){app_json_response(['ok'=>false,'message'=>$e->getMessage()],422);}
        wholesale_portal_sync_account_knowledge($pdo,$organizationId,$accountId,(int)$user['id']);
        app_audit($pdo,$organizationId,(int)$user['id'],'wholesale.quote_created','wholesale_quote',$created['publicId'],null,['accountId'=>$accountIdPublic,'quoteNumber'=>$created['quoteNumber'],'total'=>$created['totals']['total']]);
        app_json_response(['ok'=>true,'message'=>'Wholesale quote created.','quoteNumber'=>$created['quoteNumber']]);
    }
}

if($action==='order'){
    if(wholesale_commerce_ready($pdo)){
        try{$created=wholesale_commerce_create_order($pdo,$organizationId,$account,$input,(int)$user['id']);}catch(Throwable $e){app_json_response(['ok'=>false,'message'=>$e->getMessage()],422);}
        wholesale_portal_sync_account_knowledge($pdo,$organizationId,$accountId,(int)$user['id']);
        if(wholesale_demand_ready($pdo))wholesale_demand_sync_order($pdo,$organizationId,$created['publicId'],(int)$user['id']);
        if(operations_wholesale_ready($pdo))operations_sync_wholesale_tasks($pdo,$organizationId,(int)$user['id']);
        app_audit($pdo,$organizationId,(int)$user['id'],'wholesale.order_created','wholesale_order',$created['publicId'],null,['accountId'=>$accountIdPublic,'orderNumber'=>$created['orderNumber'],'total'=>$created['totals']['total']]);
        app_json_response(['ok'=>true,'message'=>'Wholesale order created and added to Operations.','orderNumber'=>$created['orderNumber']]);
    }
}
