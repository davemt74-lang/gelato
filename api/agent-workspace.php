...
try{
    ...
    if($action==='route'){
        ...
        if($isSchedulingContext){
            ...
            if(in_array($fallbackDomain,['general','restaurant_brain'],true)&&($localIntent||$confirmationIntent))app_json_response(['ok'=>true,'route'=>'api/scheduling-agent.php','domain'=>'scheduling_context']);
        }
        if($isPosContext){
