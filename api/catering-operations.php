...
if($action==='menu_save'){
    ...
    $batches=$input['batches']??null;if($batches!==null&&$batches!==''&&!is_numeric($batches))app_json_response(['ok'=>false,'message'=>'Batches must be numeric.'],422);
    if(($batches===null||$batches==='')&&$recipe&&$servings!==null&&(float)$recipe['yield_quantity']>0&&catering_operations_yield_is_servings((string)($recipe['yield_unit']??'')))$batches=round((float)$servings/(float)$recipe['yield_quantity'],3);
...
if($action==='match_recipes'){
    ...
    foreach($recipes->fetchAll() as $recipe){
        $name=mb_strtolower(trim((string)$recipe['name']),'UTF-8');if($name===''||!str_contains($leadText,$name))continue;
        $batches=$target&&(float)$recipe['yield_quantity']>0&&catering_operations_yield_is_servings((string)($recipe['yield_unit']??''))?round($target/(float)$recipe['yield_quantity'],3):null;
        $insert->execute([$organizationId,$operationId,(int)$recipe['id'],$recipe['name'],$target,$batches,'Matched from catering inquiry menu interests.',$userId,$userId,$organizationId,$operationId,(int)$recipe['id']]);$matched+=$insert->rowCount();
    }
...
if($action==='staff_save'){
    ...
    if($status!=='cancelled'){$conflict=catering_operations_staff_conflict($pdo,$organizationId,$assigned,$start,$end,$staffId);if($conflict)app_json_response(['ok'=>false,'message'=>'This employee already has an overlapping catering shift: '.$conflict['operation_title'].' ('.$conflict['shift_start_at'].' to '.$conflict['shift_end_at'].').'],409);}
