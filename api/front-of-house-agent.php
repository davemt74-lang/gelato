...
try{
    app_json_response(front_of_house_agent_handle($pdo,$user,$input));
}catch(FrontOfHouseAgentPermissionException $e){
    app_json_response(['ok'=>false,'message'=>$e->getMessage()],403);
}catch(InvalidArgumentException|DomainException $e){
    app_json_response(['ok'=>false,'message'=>$e->getMessage()],422);
}catch(Throwable $e){
    error_log('Front of House Agent failed: '.$e->getMessage());
    app_json_response(['ok'=>false,'message'=>'The Front of House Agent could not complete that request.'],500);
}
