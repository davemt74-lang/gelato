...
require_once __DIR__.'/../includes/sales-location-rollup.php';
...
try{$result=sales_import_csv($pdo,$org,$uid,$tmp,$name,$locationId,$allowDuplicate);sales_rebuild_all_location_rollups($pdo,$org,'csv');app_audit($pdo,$org,$uid,'sales.csv_imported','sales_import_batch',(string)$result['publicId'],null,['filename'=>$name,'rows'=>$result['rowCount'],'accepted'=>$result['accepted'],'rejected'=>$result['rejected'],'periodStart'=>$result['periodStart'],'periodEnd'=>$result['periodEnd'],'locationId'=>$locationId]);app_json_response(['ok'=>true,'import'=>$result,'message'=>'Sales CSV imported and normalized.'],201);}catch(InvalidArgumentException $e){app_json_response(['ok'=>false,'message'=>$e->getMessage()],422);}catch(Throwable $e){app_json_response(['ok'=>false,'message'=>'The sales CSV could not be imported.'],500);}
