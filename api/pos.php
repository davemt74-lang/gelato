...
try{
    ...
    if($action==='item.add'){pos_api_assert_ticket_mutable($pdo,$org,$public);$check=pos_add_item($pdo,$org,$public,(int)($input['priceId']??0),(float)($input['quantity']??1),(string)($input['specialInstructions']??''),$uid);app_audit($pdo,$org,$uid,'pos.item_added','pos_check',$public,null,['priceId'=>(int)($input['priceId']??0),'quantity'=>(float)($input['quantity']??1)]);app_json_response(['ok'=>true,'check'=>$check]);}
    if($action==='item.update'){pos_api_assert_ticket_mutable($pdo,$org,$public);$check=pos_update_item($pdo,$org,$public,(int)($input['itemId']??0),(float)($input['quantity']??1),(string)($input['specialInstructions']??''));app_audit($pdo,$org,$uid,'pos.item_updated','pos_check',$public,null,['itemId'=>(int)($input['itemId']??0),'quantity'=>(float)($input['quantity']??1)]);app_json_response(['ok'=>true,'check'=>$check]);}
    if($action==='item.void'){if(!$canVoid)app_json_response(['ok'=>false,'message'=>'POS void permission required.'],403);pos_api_assert_ticket_mutable($pdo,$org,$public);$reason=(string)($input['reason']??'');$check=pos_void_item($pdo,$org,$public,(int)($input['itemId']??0),$reason,$uid);app_audit($pdo,$org,$uid,'pos.item_voided','pos_check',$public,null,['itemId'=>(int)($input['itemId']??0),'reason'=>mb_substr(trim($reason),0,500,'UTF-8')]);app_json_response(['ok'=>true,'check'=>$check]);}
    if($action==='discount.set'){if(!$canDiscount)app_json_response(['ok'=>false,'message'=>'POS discount permission required.'],403);pos_api_assert_ticket_mutable($pdo,$org,$public);$reason=(string)($input['reason']??'');$amount=(float)($input['amount']??0);$check=pos_apply_discount($pdo,$org,$public,$amount,$reason);app_audit($pdo,$org,$uid,'pos.discount_updated','pos_check',$public,null,['amount'=>pos_money($amount),'reason'=>mb_substr(trim($reason),0,500,'UTF-8')]);app_json_response(['ok'=>true,'check'=>$check]);}
