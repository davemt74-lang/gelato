...
try{
    ...
    if($action==='exception.ack'){
        $key=trim((string)($in['exceptionKey']??''));$phase=manager_brief_phase((string)($in['phase']??'live'));$current=manager_brief_snapshot($pdo,$org,$date,$location['id'],$phase,$uid);$valid=array_column($current['exceptions'],'key');if(!in_array($key,$valid,true))throw new InvalidArgumentException('That operating exception is no longer present in the current brief. Refresh before acknowledging it.');
        $acks=manager_brief_ack_exception($pdo,$org,$location,$date,$key,(string)($in['note']??''),$uid);app_audit($pdo,$org,$uid,'manager.brief_exception_acknowledged','manager_daily_log',$date.'|'.$location['key'],null,['exceptionKey'=>$key,'phase'=>$phase]);app_json_response(['ok'=>true,'acknowledgements'=>$acks,'brief'=>manager_brief_snapshot($pdo,$org,$date,$location['id'],$phase,$uid)]);
    }
