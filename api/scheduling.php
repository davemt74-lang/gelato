...
if($_SERVER['REQUEST_METHOD']==='GET'){
    $action=(string)($_GET['action']??'bootstrap');$week=scheduling_week_start((string)($_GET['week']??''));$canView=sched_can($user,'schedule.view');$canSelf=sched_can($user,'schedule.self');$canStaff=sched_can($user,'staff.view')||sched_can($user,'staff.manage');if(!$canView&&!$canSelf&&!$canStaff)app_json_response(['ok'=>false,'message'=>'Scheduling permission required.'],403);
    if($action==='bootstrap'){
        $weekRow=scheduling_ensure_week($pdo,$organizationId,$week,$userId);$onlyUser=(!$canView&&$canSelf)?$userId:null;
        app_json_response(['ok'=>true,'week'=>$weekRow,'summary'=>$canView?scheduling_summary($pdo,$organizationId,$week):null,'shifts'=>($canView||$canSelf)?scheduling_shifts($pdo,$organizationId,$week,$onlyUser):[],'staff'=>$canStaff||sched_can($user,'schedule.manage')?scheduling_staff($pdo,$organizationId):[],'positions'=>scheduling_positions($pdo,$organizationId),'locations'=>scheduling_locations($pdo,$organizationId),'requests'=>sched_can($user,'schedule.manage')?scheduling_requests($pdo,$organizationId):($canSelf?scheduling_requests($pdo,$organizationId,$userId):[]),'exceptions'=>sched_can($user,'schedule.manage')?scheduling_exceptions($pdo,$organizationId):($canSelf?scheduling_exceptions($pdo,$organizationId,$userId):[]),'availability'=>$canSelf?scheduling_availability($pdo,$organizationId,$userId):[],'permissions'=>['view'=>$canView,'manage'=>sched_can($user,'schedule.manage'),'self'=>$canSelf,'staffView'=>$canStaff,'staffManage'=>sched_can($user,'staff.manage'),'agent'=>sched_can($user,'schedule.agent')]]);
    }
    if($action==='shifts'){if(!$canView&&!$canSelf)app_json_response(['ok'=>false,'message'=>'Schedule view permission required.'],403);app_json_response(['ok'=>true,'shifts'=>scheduling_shifts($pdo,$organizationId,$week,$canView?null:$userId)]);}
    if($action==='staff'){if(!$canStaff&&!sched_can($user,'schedule.manage'))app_json_response(['ok'=>false,'message'=>'Staff view permission required.'],403);app_json_response(['ok'=>true,'staff'=>scheduling_staff($pdo,$organizationId)]);}
...
try{
    ...
    if($action==='request_create'){
        if(!sched_can($user,'schedule.self')&&!sched_can($user,'schedule.manage'))app_json_response(['ok'=>false,'message'=>'Employee scheduling permission required.'],403);$row=scheduling_request_create($pdo,$organizationId,$userId,$input);app_audit($pdo,$organizationId,$userId,'schedule.shift_request_created','shift_request',(string)$row['public_id']);app_json_response(['ok'=>true,'request'=>$row,'message'=>'Shift request submitted.'],201);
    }
