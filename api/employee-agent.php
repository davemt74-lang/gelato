...
$employeeAccess=app_has_permission('employee.self',$user)||app_has_permission('tasks.self',$user)||app_has_permission('agent.employee_view',$user)||app_has_permission('schedule.self',$user)||app_has_permission('timeclock.self',$user)||app_has_permission('training.self_view',$user)||app_has_permission('employee.handoffs.view',$user);
...
$homeReady=employee_home_ready($pdo);$handoffReady=employee_shift_comms_ready($pdo);

$handoffWrite=$handoffReady&&(preg_match('/\b(?:leave|post|add|create|record|tell)\b.*\b(?:handoff|next shift|shift note|station note)\b/u',$normalized)||preg_match('/\b(?:handoff|shift note|station note)\s*[:\-]/u',$normalized));
if($handoffWrite){
    if(!app_has_permission('employee.handoffs.create',$user)&&!app_has_permission('employee.handoffs.manage',$user))app_json_response(['ok'=>false,'message'=>'Shift handoff create permission required.'],403);
    $body=preg_replace('/^hey\s+gelato[,\s]*/iu','',$message)??$message;
    if(preg_match('/(?:handoff|shift note|station note|next shift)\s*(?:that|saying|:|\-)?\s*(.+)$/iu',$body,$m)&&trim((string)$m[1])!=='')$body=trim((string)$m[1]);
    $station='';if(preg_match('/\b(?:at|for)\s+(?:the\s+)?([a-z0-9 &\-]{2,40})\s+station\b/iu',$message,$m))$station=trim((string)$m[1]);
    $row=employee_shift_message_save($pdo,$org,['title'=>'Shift handoff','body'=>$body,'station'=>$station,'priority'=>preg_match('/\b(?:urgent|critical|important|high priority)\b/u',$normalized)?'high':'normal'],$uid,false);
    app_audit($pdo,$org,$uid,'agent.employee_handoff_created','employee_shift_message',(string)$row['public_id'],null,['source'=>$in['channel']??'agent']);
    app_json_response(['ok'=>true,'skill'=>'employee.handoff.create','answer'=>'I saved that handoff for the next matching shift. It will appear in Employee Home and the arrival brief.','data'=>$row,'sources'=>[(string)$row['public_id']]]);
}

if($handoffReady&&preg_match('/\b(what happened before i got here|what happened before my shift|what changed|handoff|handoffs|shift notes?|station notes?|anything i should know|before i got here|before my shift)\b/u',$normalized)){
    $rows=employee_shift_messages_for_user($pdo,$org,$uid);$unread=array_values(array_filter($rows,static fn(array $r):bool=>empty($r['read_at'])));if(!$rows)app_json_response(['ok'=>true,'skill'=>'employee.handoff','answer'=>'There are no active shift handoffs or targeted updates for your current restaurant scope.','data'=>[],'sources'=>['employee-home.php']]);
    $lines=[];foreach(array_slice($rows,0,16) as $r){$scope=array_filter([$r['station']?('station '.$r['station']):null,$r['location_name']??null,$r['position_name']??null]);$lines[]=$r['title'].($r['priority']&&$r['priority']!=='normal'?' — '.$r['priority']:'').($scope?' — '.implode(' / ',$scope):'').': '.mb_substr((string)$r['body'],0,420,'UTF-8').($r['read_at']?'':' — unread');}
    app_audit($pdo,$org,$uid,'agent.employee_handoff_used','user',(string)$uid,null,['count'=>count($rows),'unread'=>count($unread)]);app_json_response(['ok'=>true,'skill'=>'employee.handoff','answer'=>"Here is the current shift handoff:\n- ".implode("\n- ",$lines),'data'=>$rows,'sources'=>array_column($rows,'public_id')]);
}

if($homeReady&&preg_match('/\b(my day|today|employee home|what.*need.*do|what.*doing|overview|brief me)\b/u',$normalized)){
    $d=employee_home_dashboard($pdo,$org,$uid);$next=$d['shifts'][0]??null;$clock=$d['clock']['entry']??null;$parts=[];$parts[]=$clock?'You are currently clocked in.':'You are currently off the clock.';$parts[]=$next?'Your next published shift is '.$next['title'].' on '.date('D M j, g:i a',strtotime((string)$next['starts_at'])).'.':'You have no published shift in the next two weeks.';$parts[]='You have '.count($d['tasks']).' open assigned task(s), '.(int)$d['summary']['openTraining'].' open training assignment(s), and '.(int)$d['summary']['openChecklist'].' open checklist item(s).';if($handoffReady){$h=employee_shift_messages_for_user($pdo,$org,$uid);$unread=count(array_filter($h,static fn(array $r):bool=>empty($r['read_at'])));if($unread>0)$parts[]='There are '.$unread.' unread shift handoff or targeted update(s) for you.';}if((int)$d['summary']['unreadAnnouncements']>0)$parts[]='You have '.(int)$d['summary']['unreadAnnouncements'].' unread restaurant update(s).';if((int)$d['summary']['unacknowledgedPolicies']>0)$parts[]='You have '.(int)$d['summary']['unacknowledgedPolicies'].' policy acknowledgement(s) waiting.';
...
if($homeReady&&preg_match('/\b(announcement|announcements|restaurant update|restaurant updates|message from|updates from)\b/u',$normalized)){
    $rows=employee_home_announcements($pdo,$org,$uid);if(!$rows)app_json_response(['ok'=>true,'skill'=>'employee.announcements','answer'=>'There are no current restaurant-wide announcements. Targeted shift updates appear in your handoff brief.','data'=>[],'sources'=>['employee-home.php']]);$lines=[];foreach(array_slice($rows,0,12) as $r)$lines[]=$r['title'].($r['read_at']?'':' — unread').': '.mb_substr((string)$r['body'],0,280,'UTF-8');app_json_response(['ok'=>true,'skill'=>'employee.announcements','answer'=>"Current restaurant updates:\n- ".implode("\n- ",$lines),'data'=>$rows,'sources'=>array_column($rows,'public_id')]);
}
...
app_json_response(['ok'=>true,'skill'=>'employee.context','answer'=>'I can help with your employee day, shift handoffs, arrival brief, published shifts, time clock, assigned prep and tasks, training, certifications, onboarding checklist, policies, restaurant announcements, and your employee profile. You can also say “leave a handoff for the next shift…” and I will save it in your current restaurant scope.','data'=>null,'sources'=>['employee-home.php']]);
