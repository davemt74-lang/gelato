...
require_once __DIR__.'/../includes/wholesale-demand.php';
...
if($action==='accept_quote'){
    ...
    try{
        ...
        $insert=$pdo->prepare("INSERT INTO wholesale_orders (organization_id,wholesale_account_id,source_quote_id,price_list_id,public_id,order_number,status,items_json,subtotal,delivery_fee,tax_total,tax_rate_percent,total,fulfillment_type,customer_notes,created_by,updated_by) VALUES (?,?,?,?,?,?,'requested',?,?,?,?,?,?,?,?,?,?)");
        $insert->execute([$organizationId,$accountId,(int)$quote['id'],$quote['price_list_id'],$orderPublic,$orderNumber,$quote['items_json'],$quote['subtotal'],$quote['delivery_fee'],$quote['tax_total'],$quote['tax_rate_percent'],$quote['total'],$account['preferred_fulfillment'],'Created automatically when customer accepted '.$quote['quote_number'].'.',(int)$user['id'],(int)$user['id']]);
        $orderId=(int)$pdo->lastInsertId();
        if(wholesale_commerce_ready($pdo)){wholesale_commerce_copy_quote_lines_to_order($pdo,$organizationId,(int)$quote['id'],$orderId);wholesale_commerce_order_event($pdo,$organizationId,$orderId,'created_from_quote','Order created from accepted quote.',(int)$user['id'],['quoteNumber'=>$quote['quote_number'],'orderNumber'=>$orderNumber]);}
        $pdo->commit();
    }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
    if(wholesale_demand_ready($pdo))wholesale_demand_sync_order($pdo,$organizationId,$orderPublic,(int)$user['id']);
