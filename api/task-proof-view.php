...
$storageRoot=realpath(RESTAURANT_APP_ROOT.'/storage');$absolute=realpath(RESTAURANT_APP_ROOT.'/'.ltrim((string)$proof['storage_path'],'/'));if($storageRoot===false||$absolute===false||!str_starts_with($absolute,$storageRoot.DIRECTORY_SEPARATOR)||!is_file($absolute)){http_response_code(404);exit('Proof image unavailable.');}
header('Content-Type: '.(string)$proof['mime_type']);header('Content-Length: '.(string)filesize($absolute));header('Content-Disposition: inline; filename="'.addcslashes(basename((string)$proof['original_name']),"\"\\").'"');header('Cache-Control: private, no-store, max-age=0');header('Pragma: no-cache');header('X-Content-Type-Options: nosniff');readfile($absolute);exit;
