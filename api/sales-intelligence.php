...
if($_SERVER['REQUEST_METHOD']==='GET'){
    ...
    if($action==='integrations')app_json_response(['ok'=>true,'integrations'=>sales_integrations($pdo,$org),'catalog'=>sales_provider_catalog(),'permissions'=>['manage'=>$canIntegrations]]);
...
try{
    if($action==='forecast'){
        if(!$canView)app_json_response(['ok'=>false,'message'=>'Sales Intelligence permission required.'],403);$date=trim((string)($in['date']??''));$service=(string)($in['service']??'all');$locationId=isset($in['locationId'])&&$in['locationId']!==''?(int)$in['locationId']:null;$weeks=isset($in['weeks'])?(int)$in['weeks']:8;$forecast=sales_forecast($pdo,$org,$date,$service,$locationId,$uid,$weeks);app_audit($pdo,$org,$uid,'sales.forecast_generated','sales_forecast',(string)($forecast['forecast']['public_id']??''),null,['date'=>$date,'service'=>$service,'locationId'=>$locationId,'weeks'=>$weeks]);app_json_response(['ok'=>true,'forecast'=>$forecast,'message'=>'Sales and staffing-capacity forecast generated.']);
    }
