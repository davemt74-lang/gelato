(() => {
  ...
  function isTimeclock(text) { return timeclockIntent.test(text); }
  ...
  async function submit(rawText, options = {}) {
    ...
    if (guidance) guidance.textContent = isTimeclock(text) ? 'Gelato is checking time, attendance, shifts, and employee context.' : (isScheduling(text) ? 'Gelato is checking staff schedules, availability, coverage, and workload.' : (isOperations(text) ? 'Gelato is checking tasks, prep, inventory, and fulfillment.' : 'Restaurant Agent is checking private business knowledge.'));
    try {
      const endpoint = isTimeclock(text) ? 'api/timeclock-agent.php' : (isScheduling(text) ? 'api/scheduling-agent.php' : (isOperations(text) ? 'api/operations-agent.php' : 'api/agent-brain.php'));
      const response = await fetch(endpoint,{method:'POST',cache:'no-store',headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-Token':window.RESTAURANT_CSRF_TOKEN || ''},body:JSON.stringify({action:'ask',message:text,voice:!!options.voice,voiceEventId:options.voiceEventId||'',csrf_token:window.RESTAURANT_CSRF_TOKEN || ''})});
      ...
      if (pending) pending.remove(); const message = error.message || 'The Restaurant Agent is unavailable.'; saveMessage('agent', message); appendMessage('agent', message); if (guidance) guidance.textContent = 'Restaurant Agent request could not be completed.'; throw error;
    } finally { busy = false; }
  }
  window.GelatoRestaurantAgent = { submit };
  function maybeSubmit(event) {
    ...
    event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation(); submit(text).catch(()=>{}); return true;
  }
  function installVoice() {
    ...
    button.addEventListener('click',()=>{const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){const guidance=document.getElementById('ownerComposerGuidance');if(guidance)guidance.textContent='Voice recognition is not available in this browser.';return;}const rec=new SR();rec.lang='en-US';rec.interimResults=false;rec.continuous=false;button.textContent='●';rec.onresult=e=>{const text=[...e.results].map(r=>r[0].transcript).join(' ');input.value=text;if(shouldHandle(text))submit(text).catch(()=>{});};rec.onerror=e=>{const guidance=document.getElementById('ownerComposerGuidance');if(guidance)guidance.textContent='Voice recognition error: '+e.error;};rec.onend=()=>button.textContent='🎙';rec.start();});
    ...
    if (Auth.has('timeclock.view') || Auth.has('timeclock.self')) { const clockLink=document.createElement('a');clockLink.href='timeclock.php';clockLink.textContent='Time Clock';clockLink.style.cssText=opsLink.style.cssText;send?.parentNode?.appendChild(clockLink); }
  }
  ...
  const input = document.getElementById('ownerAgentInput'); if (input) input.placeholder = 'Ask or say: Hey Gelato… who is clocked in, schedule Anthony, show coverage gaps, add to prep list…';
  installVoice();
})();