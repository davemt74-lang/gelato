...
<head>
  ...
  <link rel="stylesheet" href="assets/css/site.css?v=20261001">
</head>
<body>
...
<main id="top">
  <section class="hero">
    ...
    <div class="shell hero-inner">
      <div class="hero-copy" data-reveal>
        ...
        <div class="hero-actions"><a class="btn btn-primary" href="menu.php">View Our Menu</a><a class="btn btn-secondary" href="locations.php">Visit Us</a></div>
      </div>
    </div>
  </section>
  <div class="feature-wrap"><div class="shell feature-grid">
    ...
    <article class="feature-card" data-reveal><img src="<?= app_escape(public_site_asset('card-music.jpg')) ?>" alt="Community night at Stonefellows"><div class="feature-content"><div class="eyebrow">Community</div><h3>More Than a Meal.</h3><p>A local gathering place built for friends, families, teams and neighborhood nights.</p><a class="btn-link" href="about.php">Our Story →</a></div></article>
    <article class="feature-card" data-reveal><img src="<?= app_escape(public_site_asset('card-reservations.jpg')) ?>" alt="Dining table at Stonefellows"><div class="feature-content"><div class="eyebrow">Visit</div><h3>Your Table Is Waiting.</h3><p><?= $address !== '' ? app_escape($address) : 'Location and contact details are managed from the restaurant admin.' ?></p><a class="btn-link" href="locations.php">Locations →</a></div></article>
  </div></div>
  <section class="section" id="story"><div class="shell story-grid" data-reveal><div class="story-copy"><div class="eyebrow">Our Story</div><h2>Built on Pizza.<br>Fueled by People.</h2><p>Stonefellows is a neighborhood gathering place built around simple things done well: great dough, a hot oven, cold drinks, warm hospitality and a room that feels better when it is full.</p><a class="btn btn-secondary" href="about.php">About Stonefellows</a></div><img class="story-image" src="<?= app_escape(public_site_asset('story-one.jpg')) ?>" alt="Stonefellows dining room"><img class="story-image" src="<?= app_escape(public_site_asset('story-two.jpg')) ?>" alt="Stonefellows bar and pizza oven"></div></section>
  ...
  <section class="section-tight" id="favorites"><div class="shell"><div class="section-head"><div><div class="eyebrow">From the Current Menu</div><h2>Featured Pizza</h2></div><a class="btn-link" href="menu.php">View Full Menu →</a></div>
  <?php if ($featuredPizzas): ?><div class="favorites-grid"><?php foreach ($featuredPizzas as $index => $item): ?><article class="menu-card" data-reveal><img src="<?= app_escape(public_site_asset($favoriteImages[$index] ?? $favoriteImages[0])) ?>" alt="<?= app_escape((string)$item['name']) ?>"><div class="menu-card-body"><h3><?= app_escape((string)$item['name']) ?></h3><?php if (trim((string)$item['description']) !== ''): ?><p><?= app_escape((string)$item['description']) ?></p><?php endif; ?><?php $price = public_site_price($item); if ($price !== ''): ?><span class="price"><?= app_escape($price) ?></span><?php endif; ?></div></article><?php endforeach; ?></div><?php else: ?><div class="empty-state">The pizza menu has not been published to the restaurant database yet.</div><?php endif; ?></div></section>
...
<script src="assets/js/site.js?v=20261001"></script>
</body></html>
